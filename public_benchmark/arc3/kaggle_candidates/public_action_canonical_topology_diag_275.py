...
Falsifier-driven change from R274: R274 removed absolute position but still fit each
action label separately and found NO_SIGNAL. R275 uses the action itself as a spatial
symmetry operator: directional transitions are rotated into a common UP-oriented frame
and pooled under one MOVE action class. This tests whether equivalent mechanisms are
being fragmented by screen orientation/action label rather than by object topology.

Protocol: p0-p4 fit -> p5-p9 diagnostic only. p10-p19 are not staged/read.
PUBLIC_OFFLINE only; no Kaggle/hidden performance claim.
"""
...
def canon_board(board, action, use_ui_mask=False):
    ...
    if a=='LEFT': return rot_cw(b)       # LEFT vector -> UP
    if a=='RIGHT': return rot_ccw(b)     # RIGHT vector -> UP
    return b


def action_class(action):
    a=str(action).upper()
    return 'MOVE' if a in DIRS else a


def key(board, action, mode):
    if mode=='raw':
        return r246.digest(board), str(action).upper()
    use_ui=mode.endswith('_ui')
    b=canon_board(board,action,use_ui_mask=use_ui)
    base=mode[:-3] if use_ui else mode
    if base=='canon_raw':
        d=r246.digest(b)
    elif base=='canon_nodes':
        d=r274.dig(r274.desc(b,'nodes_coarse'))
    elif base=='canon_graph':
        d=r274.dig(r274.desc(b,'graph_coarse_noedge'))
    else:
        raise KeyError(mode)
    return d, action_class(action)
...
def fit(rows, mode):
    ...
    for r in rows:
        ...
        n=key(r['after'],r['action'],mode)[0]
        obs[k][n]+=1; counts[k]+=1
    tab={k:next(iter(v)) for k,v in obs.items() if len(v)==1}
    return tab, {
        'keys':len(obs),'deterministic':len(tab),'ambiguous':sum(len(v)>1 for v in obs.values()),
...
def main():
    ...
    for g,ps in sorted(by.items()):
        ...
        for m in modes[1:]:
            v=mm[m]['validation']
            if int(v.get('wrong',0))==0 and int(v.get('correct',0))>int(raw.get('correct',0)):
                candidates.append(m)
        if candidates:
    ...
    out={
        'schema':'deus/arc3-r275-action-canonical-topology-diagnostic/1','rung':275,
        'representation_delta':'directional actions are spatially canonicalized to UP and pooled as MOVE; optional static UI mask; component topology is evaluated in the action-aligned frame',
        'lineage':{'r273':'absolute object state NO_SIGNAL','r274':'translation-invariant topology NO_SIGNAL','repair':'use action as a symmetry transform and pool directional mechanisms rather than retuning static topology'},
        'protocol':{'fit':'p0-p4','diagnostic':'p5-p9','p10_p19_staged_or_read':False,'promotion_in_r275':False},
        'unresolved_games':sorted(by),'signals':signals,'games':games,'verdict':'DIAGNOSTIC_SIGNAL' if signals else 'NO_SIGNAL',
        'truth':{'public_trace_only':True,'source_free_runtime_logic':True,'p10_p19_read':False,'independent_hidden_generalization_claim':False,'kaggle_execution':False,'competition_submission':False,'submission_quota_spent_by_r275':False}
    }
