#!/usr/bin/env python3
"""Rung 124: target-blind recency anti-loop / soft-exploration routing A/B.

Rung 123 showed that a full action-utility table added prompt/latency cost without level
gain. This rung changes the representation rather than retrying that table: both arms
keep the exact current visible frame, explicit no-new-frame carry semantics and the same
same-run effect memory. The treatment adds only a compact routing prior when an action
has two consecutive *visible* no-effect outcomes. That action is softly demoted and
unseen legal actions are softly preferred for exploration; no legal action is removed.
Unknown/no-new-frame outcomes are never counted as negative evidence.

Conceptual source grounding (clean-room implementation):
- GameDevGitHub/GuidedRandomAgent: action-effectiveness bias and reduced probability for
  repeatedly ineffective actions (MIT).
- khub-ai/arc-agi-3: update beliefs from observed outcomes and use context-aware
  exploration / means-ends routing (MIT-0).
"""
...
TARGETS = em.TARGETS
ARMS = ("effect_memory", "effect_memory_plus_recency_prior")
STEP_BUDGET = 4
...
SOURCE_GROUNDING = {
    "guided_random": {
        ...
        "license": "MIT",
        "principle": "softly reduce probability of repeatedly ineffective observed actions",
    },
    "khub": {
        "upstream_ref": "khub-ai/arc-agi-3@de41a93e3a4557db24702fceddb2409c7bc06afd",
        "license": "MIT-0",
        "principle": "observed-outcome world-model evidence with context-aware exploration",
    },
...
def recency_prior(memory: list[dict[str, Any]], legal_ids: list[int]) -> dict[str, Any] | None:
    soft_demote: list[int] = []
    attempted = {int(r.get("action", -1)) for r in memory}
    for aid in legal_ids:
        visible = []
        for row in memory:
            if int(row.get("action", -1)) != int(aid):
                continue
            if int(row.get("level_delta", 0) or 0) > 0:
                visible.append("GAIN")
                continue
            if not bool(row.get("new_frame_observed")):
                continue
            changed = row.get("settled_delta", {}).get("changed")
            if changed is None:
                continue
            visible.append("EFFECT" if int(changed) > 0 else "NO_EFFECT")
        if len(visible) >= 2 and visible[-2:] == ["NO_EFFECT", "NO_EFFECT"]:
            soft_demote.append(int(aid))
    if not soft_demote:
        return None
    unseen = [int(a) for a in legal_ids if int(a) not in attempted]
    return {
        "soft_demote_after_two_consecutive_visible_no_effects": sorted(soft_demote),
        "soft_explore_unseen": sorted(unseen),
...
def prompt_for(game_id: str, env: Any, obs: Any, current_frame: Any, current_fresh: bool,
    ...
    if arm == "effect_memory_plus_recency_prior":
        prior = recency_prior(memory, [int(a["id"]) for a in actions])
        if prior is not None:
            payload["routing_prior"] = prior
            payload["routing_note"] = (
                "This is a soft anti-loop prior only. Soft-demoted actions remain legal and may still be chosen; "
                "prefer unseen alternatives only when plausible from the exact current frame."
            )
...
def run_arm(arcade: Any, game_id: str, probe_action: int, arm: str) -> dict[str, Any]:
    ...
    for step in range(1, STEP_BUDGET + 1):
        ...
        prior = recency_prior(memory, legal_ids) if arm == "effect_memory_plus_recency_prior" else None
        if prior is not None:
...
def main() -> int:
    ...
    ap.add_argument("--receipt", type=Path, default=Path("public-recency-anti-loop-ab.json"))
    args = ap.parse_args()
    ...
    aug = summarize(results, "effect_memory_plus_recency_prior")
    equal_positive = (
    ...
    if inconclusive:
        ...
        gate = "PROMOTE_RECENCY_ANTI_LOOP_MORE_GAINS"
    elif equal_positive and nondominated_efficiency:
        gate = "PROMOTE_RECENCY_ANTI_LOOP_EQUAL_POSITIVE_NONDOMINATED"
    else:
        gate = "VALID_NO_PROMOTION"

    receipt = {
        "schema": "deus/arc3-public-recency-anti-loop-ab/1",
        "toolkit": "arc-agi==0.9.9",
        "change_kind": "COMPACT_ROUTING_REPRESENTATION_NOT_ACTION_RETRY",
        ...
        "summary": {
            "effect_memory": base,
            "effect_memory_plus_recency_prior": aug,
        ...
        "truth": {
            ...
            "candidate_changes_routing_representation_not_pixel_geometry": True,
            "prior_requires_two_consecutive_visible_no_effects": True,
