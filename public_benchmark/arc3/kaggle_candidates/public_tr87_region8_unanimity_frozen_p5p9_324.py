#!/usr/bin/env python3
"""R324: frozen TR87 region8+cross-trace-unanimity p5-p9 replay.

Candidate identity was fixed by R323 using public p0-p4 only. R324 does not
select a new representation or reliability rule: it refits the exact frozen
R323 rule family on all p0-p4 traces and evaluates unchanged on p5-p9.
No p10-p19 traces are staged/read.

PUBLIC_OFFLINE validation only; not hidden generalization, whole-game solve,
Kaggle execution, competition submission, or leaderboard score.
"""
...
def main():
    ...
    total = Counter()
    per_trace = []
    for i, prepared in enumerate(prepared_eval, start=5):
    ...
    if total.get("predicted_changes", 0) > 0 and total.get("false_changes", 0) == 0 and total.get("pixel_gain", 0) > 0 and total.get("exact_frame_gain", 0) >= 0:
        verdict = "FROZEN_P5P9_ZERO_FALSE_PASS"
    elif total.get("pixel_gain", 0) > 0:
        verdict = "FROZEN_P5P9_GAIN_WITH_FALSE_CHANGE"
    else:
        verdict = "FROZEN_P5P9_NO_SIGNAL"

    out = {
        "schema": "deus/arc3-r324-tr87-region8-unanimity-frozen-p5p9/1",
        "rung": RUNG,
        "game": GAME,
        "candidate": {
            "representation": "R321 delta+region8",
            "reliability": "R323 deterministic rule supported in every p0-p4 training trace",
            "selection_source": "R323 p0-p4 LOTO only",
            "representation_changed_after_r323": False,
            "reliability_changed_after_r323": False,
        },
        "protocol": {
            "fit": "exact public p0-p4 all five traces",
            "evaluation": "exact public p5-p9 frozen replay",
            "retune_on_p5_p9": False,
