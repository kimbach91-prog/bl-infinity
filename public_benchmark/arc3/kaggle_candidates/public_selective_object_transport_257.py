...
Grounding from R256: movement actions show small-object transport in >95% of
transitions, with strong action-aligned modal vectors (UP≈[-3,0], DOWN≈[3,0],
LEFT≈[0,-3], RIGHT≈[0,3]) even though R255 falsified whole-frame viewport
translation. R257 tests the discriminating mechanism directly: translate only
visible small object classes whose matched p0-p4 behavior consistently follows
the action's learned dominant vector; leave the rest screen-fixed.

Protocol:
  p0-p4 fit transport rules
  p5-p9 select per action only if added full-frame predictions >0 and ZERO wrong
...
No game source, hidden data, Kaggle runtime/score or leaderboard information is
read. This is PUBLIC_OFFLINE/public-development evidence only.
"""
...
RUNG=257
VARIANTS=(
    (1,0.80,3),(1,0.90,3),(1,1.00,3),
    (13,0.80,3),(13,0.90,3),(13,1.00,3),
    (36,0.80,3),(36,0.90,3),(36,1.00,3),
    (36,0.90,5),
)


def cls(comp):
    return (int(comp['color']), int(comp['area']), r246.stable(comp['shape']))
...
def learn(rows,max_area:int,min_frac:float,min_support:int):
    # First infer a dominant non-zero action vector from matched small components.
    vec=Counter()
    ...
    for row in rows:
        ...
        for b,a in matches:
            ...
            d=(a['r0']-b['r0'],a['c0']-b['c0'])
            per[cls(b)][d]+=1
            if d!=(0,0): vec[d]+=1
    if not vec: return None,{}
    dominant=vec.most_common(1)[0][0]
    movable={}
    for k,c in per.items():
        support=sum(c.values())
        frac=c[dominant]/support if support else 0.0
        if support>=min_support and frac>=min_frac and dominant!=(0,0):
            movable[k]=support
...
def render(board,vector,movable,max_area:int):
    ...
    h=len(board); w=len(board[0]); bg=r246.bg(board)
    # Fail closed if any moved component exits screen.
    dr,dc=vector
    for c in comps:
        if c['r0']+dr<0 or c['c0']+dc<0 or c['r1']+dr>=h or c['c1']+dc>=w: return None
    out=[list(map(int,row)) for row in board]
    # Clear moved object cells to the observed dominant background, then redraw.
    for c in comps:
        for rr,cc in c['shape']:
            out[c['r0']+rr][c['c0']+cc]=int(bg)
    for c in comps:
        for rr,cc in c['shape']:
...
def eval_rows(rows,exact,model,max_area):
    ...
    for row in rows:
        ...
        p=render(row['before'],vector,movable,max_area)
        if p is None:
        ...
        if len(examples)<20:
            examples.append({'trace':row['trace'],'action':row['action'],'correct':ok,'vector':list(vector) if vector else None})
...
def evaluate_game(paths):
    ...
    for action in sorted(set(tr_by)|set(va_by)):
        ...
        for max_area,min_frac,min_support in VARIANTS:
            model=learn(tr_by[action],max_area,min_frac,min_support)
            met=eval_rows(va_by[action],exact_tr,model,max_area)
            key=f'a{max_area}_f{int(min_frac*100)}_s{min_support}'
            cand[key]={'max_area':max_area,'min_frac':min_frac,'min_support':min_support,
                       'vector':list(model[0]) if model[0] else None,'movable_class_count':len(model[1]),'validation':met}
        zero=[k for k,v in cand.items() if v['validation'].get('candidate_predictions',0)>0 and v['validation'].get('candidate_wrong',0)==0]
        if zero:
            zero.sort(key=lambda k:(-cand[k]['validation'].get('candidate_correct',0),-cand[k]['validation'].get('candidate_predictions',0),k))
    ...
    for action,k in selected.items():
        v=diag[action][k]
        refit[action]=(v['max_area'],learn(fit_by[action],v['max_area'],v['min_frac'],v['min_support']))
    ...
    for row in ho:
        ...
        if row['action'] not in refit:
            s['candidate_abstain']+=1; continue
        max_area,model=refit[row['action']]
        p=render(row['before'],model[0],model[1],max_area)
        if p is None:
            s['candidate_abstain']+=1; continue
        ...
        if len(examples)<30:
            examples.append({'trace':row['trace'],'action':row['action'],'variant':selected[row['action']],
                             'vector':list(model[0]) if model[0] else None,'correct':ok})
    n=s['candidate_predictions']; opp=s['baseline_abstain']
...
def main():
    ...
    out={'schema':'deus/arc3-r257-selective-object-transport/1','rung':RUNG,
         'lineage':{'r256':'run35798689155/artifact10725086548','repair':'selective action-conditioned object transport; whole-screen shift remains falsified'},
         ...
         'truth':{'public_trace_only':True,'game_source_read':False,'selection_uses_p0_p9_only':True,
                  ...
                  'independent_generalization_claim':False,'kaggle_execution':False,'competition_submission':False,'submission_quota_spent':False,'owner_score_claim':False}}
