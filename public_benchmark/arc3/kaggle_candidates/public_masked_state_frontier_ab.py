#!/usr/bin/env python3
"""Public UI-masked state-frontier A/B.

Source-grounding: shloksah/arc-agi-3-agent@7dccef15650fa00ef1db1c21cc7279adcf4a9e2a
(MIT-0) reports that ARC engine UI counters/pips on the outer ring and row 1 can
make identical play states hash differently. This clean-room rung tests only that
representation idea: a state key hashes the exact visible final frame after replacing
outer-ring cells and row 1 with a sentinel. No game-specific source or answers are
used.

Both arms retain the exact current final frame and the same recent settled-effect
ledger. Treatment additionally exposes per-action effect counts observed previously
from the same UI-masked visible-state key. The key is explicitly not a hidden-state
identity claim. Promotion requires real public level gain or equal positive gain with
fewer actions; activation, action diversity, latency, or footprint cannot promote.
"""
...
TARGETS = v1.TARGETS
ARMS = ("effect_memory", "effect_memory_masked_state_frontier")
STEP_BUDGET = 10
MEMORY_LIMIT = 8
SENTINEL = 99
UPSTREAM_REF = "shloksah/arc-agi-3-agent@7dccef15650fa00ef1db1c21cc7279adcf4a9e2a"
UPSTREAM_LICENSE = "MIT-0"


def masked_state_fp(frame: Any) -> str:
    rows = [list(map(int, r)) for r in frame]
    if not rows:
        return "empty"
    h = len(rows)
    ...
    for x in range(w):
        ...
        if h > 1:
            rows[1][x] = SENTINEL
        rows[h - 1][x] = SENTINEL
    payload = v1.compact_json(rows).encode("utf-8")
...
def make_entry(action_id: int, before_frame: Any | None, before_levels: int, obs: Any) -> dict[str, Any]:
    ...
    base["before_masked_state_fp"] = masked_state_fp(before_frame) if before_frame is not None else None
    base["after_masked_state_fp"] = masked_state_fp(after) if after is not None else None
    base["level_delta"] = v1.obs_levels(obs) - int(before_levels)
    return base


def frontier(current_frame: Any, legal_ids: list[int], memory: list[dict[str, Any]]) -> dict[str, Any]:
    fp = masked_state_fp(current_frame)
    rows = [m for m in memory if m.get("before_masked_state_fp") == fp]
    by_action: dict[int, dict[str, int]] = {}
    for m in rows:
        a = int(m["action"])
        if a not in legal_ids:
            continue
        rec = by_action.setdefault(a, {"attempts": 0, "zero_effect": 0, "changed_effect": 0, "level_gain": 0})
        rec["attempts"] += 1
        ch = (m.get("settled_delta") or {}).get("changed")
        if ch == 0:
            rec["zero_effect"] += 1
    ...
    tried = set(by_action)
    return {
        "ui_masked_visible_state_fp": fp,
        "prior_observations_from_same_masked_visible_state": len(rows),
        "per_action": [{"action": a, **by_action[a]} for a in sorted(by_action)],
        "legal_untried_from_same_masked_visible_state": [a for a in sorted(legal_ids) if a not in tried],
    }
...
def prompt_for(game_id: str, env: Any, obs: Any, arm: str, memory: list[dict[str, Any]]) -> tuple[str, bool]:
    ...
    payload: dict[str, Any] = {
        ...
        "memory_note": "Observed same-run causal history; settled_delta compares persistent final frames.",
    }
    active = False
    if arm == "effect_memory_masked_state_frontier":
        f = frontier(current, legal_ids, memory)
        if f["prior_observations_from_same_masked_visible_state"] > 0:
            payload["ui_masked_state_action_frontier"] = f
            payload["frontier_note"] = (
                "Outer-ring/row-1 UI cells are ignored only for grouping. This is visible-state evidence, "
                "not proof of hidden-state identity or an instruction to avoid any action."
            )
            active = True
...
def run_arm(arcade: Any, game_id: str, probe_action: int, arm: str) -> dict[str, Any]:
    ...
    for step in range(1, STEP_BUDGET + 1):
        ...
        prompt, active = prompt_for(game_id, env, obs, arm, memory)
        call = v1.provider_action_call(prompt, legal_ids)
        row: dict[str, Any] = {"step": step, "arm": arm, "prompt_chars": len(prompt), "masked_frontier_active": active, **call}
        ...
        row["execution"] = {
            ...
            "before_masked_state_fp": entry["before_masked_state_fp"],
            "after_masked_state_fp": entry["after_masked_state_fp"],
        }
        rows.append(row)
        if observation_lost:
            break
    ...
    return {
        ...
        "masked_frontier_activations": sum(bool(r.get("masked_frontier_active")) for r in rows),
    }
...
def summarize(results: list[dict[str, Any]], arm: str) -> dict[str, Any]:
    ...
    return {
        ...
        "masked_frontier_activations_total": sum(int(r["result"].get("masked_frontier_activations", 0)) for r in valid),
...
def main() -> int:
    ...
    ap.add_argument("--receipt", type=Path, default=Path("public-masked-state-frontier-ab.json"))
    args = ap.parse_args()
    ...
    inconclusive = [{"game_id": r["game_id"], "arm": r["arm"], "reason": r["result"].get("inconclusive")} for r in results if r["result"].get("inconclusive")]
    base = summarize(results, "effect_memory")
    aug = summarize(results, "effect_memory_masked_state_frontier")
    if inconclusive:
        ...
        gate = "PROMOTE_MASKED_STATE_FRONTIER_MORE_GAINS"
    elif aug["games_with_level_gain"] == base["games_with_level_gain"] and aug["games_with_level_gain"] > 0 and aug["policy_actions_total"] < base["policy_actions_total"]:
        gate = "PROMOTE_MASKED_STATE_FRONTIER_EQUAL_GAINS_FEWER_ACTIONS"
    else:
        changed_games = 0
        for game_id, _ in TARGETS:
            b = next(r for r in results if r["game_id"] == game_id and r["arm"] == "effect_memory")
            a = next(r for r in results if r["game_id"] == game_id and r["arm"] == "effect_memory_masked_state_frontier")
            if [x.get("chosen_action") for x in b["result"].get("rows", [])] != [x.get("chosen_action") for x in a["result"].get("rows", [])]:
                changed_games += 1
        gate = "VALID_BEHAVIOR_SIGNAL_NO_PROMOTION" if changed_games or aug["masked_frontier_activations_total"] else "VALID_NO_PROMOTION"
    receipt = {
        "schema": "deus/arc3-public-masked-state-frontier-ab/1",
        "toolkit": "arc-agi==0.9.9",
        "source_grounding": {"upstream_ref": UPSTREAM_REF, "license": UPSTREAM_LICENSE, "borrowed_idea": "exclude ARC UI border/row-1 cells from visible-state hashing"},
        "targets": [{"game_id": g, "probe_action": a} for g, a in TARGETS],
        "step_budget_per_arm": STEP_BUDGET,
        "results": results,
        "summary": {"effect_memory": base, "effect_memory_masked_state_frontier": aug},
        "promotion_gate": gate,
        ...
        "truth": {
            ...
            "masked_key_not_claimed_as_hidden_state_identity": True,
            "source_license_verified_mit0": True,
