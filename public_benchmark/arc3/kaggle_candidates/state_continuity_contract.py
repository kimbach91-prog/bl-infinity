#!/usr/bin/env python3
"""Bounded local continuity state for ARC-AGI-3 candidate harnesses.

Truth boundary: deterministic CPU state primitive only. No model execution,
hidden Kaggle evaluation, submission, or leaderboard score is performed here.
"""
...
def _discoveries(raw: Any) -> Dict[str, List[str]]:
    raw = raw if isinstance(raw, dict) else {}
    return {kind: _dedupe(raw.get(kind, [])) for kind in DISCOVERY_KINDS}


class ContinuityContract:
    """Fail-closed accepted-state ledger with bounded deterministic compaction."""

    def __init__(self, game_id: str, *, max_state_chars: int = 4096,
                 summary_char_budget: int = 1400, exact_tail_turns: int = 2) -> None:
        if not game_id:
            raise ValueError("game_id is required")
        if max_state_chars < 1600 or summary_char_budget < 400 or exact_tail_turns < 1:
            raise ValueError("invalid continuity bounds")
        self.max_state_chars = max_state_chars
        self.summary_char_budget = summary_char_budget
        self.exact_tail_turns = exact_tail_turns
        self._state = self._fresh(game_id)

    @staticmethod
    def _fresh(game_id: str) -> Dict[str, Any]:
        return {
    ...
    def propose_turn(self, *, observation: Any, model_output: Any, action: Any,
                     discoveries: Any = None) -> Dict[str, Any]:
        return {
            "schema_version": SCHEMA_VERSION,
            ...
            "turn": {
                ...
                "discoveries": _discoveries(discoveries),
            },
        }
    ...
    def commit_turn(self, provisional: Dict[str, Any], *, action_is_valid: bool) -> bool:
        ...
        turn["discoveries"] = _discoveries(turn.get("discoveries"))

        accepted_before = copy.deepcopy(self._state)
        try:
            self._state["accepted_turns"].append(turn)
            if self._size() > self.max_state_chars:
                self._compact()
            if self._size() > self.max_state_chars:
    ...
    def _compact(self) -> None:
        ...
        prefix, tail = turns[:-self.exact_tail_turns], turns[-self.exact_tail_turns:]
        summary = copy.deepcopy(self._state["summary"])
        for turn in prefix:
            disc = _discoveries(turn.get("discoveries"))
            for kind in DISCOVERY_KINDS:
                summary[kind] = _dedupe([*summary[kind], *disc[kind]])
        while len(json.dumps(summary, sort_keys=True)) > self.summary_char_budget:
            candidates = [(kind, len(values)) for kind, values in summary.items() if values]
            if not candidates:
                break
            summary[max(candidates, key=lambda pair: pair[1])[0]].pop(0)
        if len(json.dumps(summary, sort_keys=True)) > self.summary_char_budget:
...
def self_test() -> Dict[str, Any]:
    ...
    before = state.digest()
    bad = state.propose_turn(observation="frame 0", model_output="bad", action="",
                             discoveries={"rules": ["red blocks movement"]})
    checks["invalid_action_is_transactional"] = (
        not state.commit_turn(bad, action_is_valid=False) and state.digest() == before
    )

    p1 = state.propose_turn(
        observation="frame 1", model_output="ACTION1", action="ACTION1",
        discoveries={"rules": ["red blocks movement"],
                     "goals": ["reach marked exit"],
    ...
    for i in range(2, 10):
        p = state.propose_turn(
            ...
            discoveries={
                "rules": ["red blocks movement"],
                "goals": ["reach marked exit"],
                "action_effects": [f"action effect {i}"],
    ...
    checks["rule_goal_survive_compaction"] = (
        "red blocks movement" in summary_blob and "reach marked exit" in summary_blob
    )
    checks["state_bound_enforced"] = state._size() <= state.max_state_chars
    checks["json_serializable"] = isinstance(json.dumps(snap), str)
    checks["no_score_or_submission_fields"] = not any(
        key in snap for key in ("score", "leaderboard", "submission")
    )

    # One large turn fits; a second would violate the protected two-turn tail.
    # The rejected second commit must leave the first accepted state untouched.
    tiny = ContinuityContract("tiny", max_state_chars=2400,
                              summary_char_budget=400, exact_tail_turns=2)
    t1 = tiny.propose_turn(observation="a" * 700, model_output="b" * 700,
                           action="ACTION1", discoveries={"rules": ["r" * 300]})
    tiny.commit_turn(t1, action_is_valid=True)
    tiny_before = tiny.digest()
    t2 = tiny.propose_turn(observation="c" * 700, model_output="d" * 700,
                           action="ACTION2", discoveries={"goals": ["g" * 300]})
    try:
        tiny.commit_turn(t2, action_is_valid=True)
    ...
    checks["cross_game_reset"] = (
        reset["game_id"] == "game-B" and not reset["accepted_turns"]
        and reset["compacted_turn_count"] == 0 and state.digest() != old_digest
    )
