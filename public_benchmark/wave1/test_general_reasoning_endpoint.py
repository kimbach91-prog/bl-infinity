...
with tempfile.TemporaryDirectory() as d:
 ...
 r=subprocess.run(["python",str(root/"general_reasoning_endpoint.py"),"--task",str(p),"--out",str(out)],capture_output=True,text=True)
 assert r.returncode==3
 x=json.loads(out.read_text())
 assert x["execution_state"]=="HOLD_NO_BOUND_SOLVER_COMMAND"
 cmd="python -c \"print('CANARY_SOLVED')\""
 r=subprocess.run(["python",str(root/"general_reasoning_endpoint.py"),"--task",str(p),"--solver-cmd",cmd,"--out",str(out)],capture_output=True,text=True)
