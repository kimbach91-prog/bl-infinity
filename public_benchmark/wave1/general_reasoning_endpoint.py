...
BACKENDS = {"BL_INFER","LOCAL_OPENAI_COMPAT","ARGV","LEGACY_SHELL"}
...
def main():
    ...
    ap.add_argument("--backend", default=os.environ.get("DEUS_BENCH_BACKEND","ARGV"))
    ...
    receipt={
      ...
      "claim_boundary":"Execution receipt only; backend liveness/identity and independent evaluator determine verification/promotion."
    }
    pathlib.Path(args.out).write_text(json.dumps(receipt,indent=2)+"\n")
    print(json.dumps(receipt,indent=2))
