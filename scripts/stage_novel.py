...
ERA_PATCH_SOURCE = ROOT / "content" / "novel" / "04_CHAPTER_001_ERA_LANGUAGE_PATCH.md"
MACHINE = ROOT / "machine"
CANONICAL_BASE = "https://kimbach91-prog.github.io/bl-infinity/"
LASTMOD = "2026-09-02"


def inject_era_language_patch(markdown_text: str) -> str:
    """Inject the provenance-separated HALF-CANON texture patch into Chapter 1."""
    if "Những năm bắt đầu có giọng riêng" in markdown_text:
        return markdown_text
    patch = ERA_PATCH_SOURCE.read_text(encoding="utf-8").strip()
    marker = "\n* * *\n\nCó những đêm điện tắt hoặc ánh sáng chập chờn"
    if marker not in markdown_text:
        raise RuntimeError("Chapter 1 era-language insertion marker missing")
...
<meta name="description" content="Chương 1 HALF-CANON của Bách Lâm: Lần Hồi Quy Thứ Một Triệu. Locked core, world build Việt Nam đầu thế kỷ XXI, era-language social texture và các cơ chế sâu vẫn giữ UNKNOWN khi chưa đủ căn cứ.">
...
def enrich_manifest() -> None:
    ...
    manifest["novel_era_language_style_registry"] = "novel-era-language-style-registry.json"
    manifest["novel_half_canon_page"] = "../novel/chapter-001.html"
    manifest["chapter_001_world_guide"] = "../novel/world-chapter-001.html"
    manifest["novel_era_language_style_atlas"] = "../novel/era-language-style-atlas.html"
    manifest["system_hub"] = "../system.html"
    manifest["open_academic_publishing_page"] = "../open-academic-publishing.html"
    manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def patch_novel_index() -> None:
    index_path = SITE / "novel" / "index.html"
    if not index_path.exists():
        return
    text = index_path.read_text(encoding="utf-8")
    if "era-language-style-atlas.html" not in text:
        block = '''<section class="canon-card" id="era-language-atlas">
<h2>Atlas ngôn ngữ xã hội & bút pháp theo thời kỳ</h2>
<p>Worldbuilding theo năm không dùng một giọng Việt Nam bất biến. Scene được resolve theo thời gian, vùng, thế hệ, quan hệ, medium và literary mode; có anachronism audit trước khi khóa texture.</p>
<p><a href="era-language-style-atlas.html">Mở Era-Language & Literary Style Atlas →</a></p>
</section>'''
        if "</main>" in text:
...
def main() -> None:
    ...
    shutil.copyfile(PUBLIC_NOVEL / "era-language-style-atlas.html", target_dir / "era-language-style-atlas.html")
    ...
    for name in (
        ...
        "novel-era-language-style-registry.json",
    ...
    markdown_text = inject_era_language_patch(markdown_text)
    ...
    append_sitemap((
        ...
        "novel/era-language-style-atlas.html",
        "system.html",
        "open-academic-publishing.html",
    ))
    print("Serialized novel HALF-CANON v1.1 + era-language patch + literary atlas + reality world guide staged")
