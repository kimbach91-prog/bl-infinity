# 03 — Các tiên đề lõi BL∞

Origin build dùng các tiên đề ở mức tối thiểu. Một số tiên đề là quy ước ontology; một số là research hypothesis mạnh hơn. Chúng phải được giữ tách biệt.

## BL-A01 — Tiên đề Nội tại

Nếu \(A\) là một chủ thể thực sự tồn tại trong \(\Omega\), mọi trạng thái thực sự của A là trạng thái thuộc \(\Omega\):

\[
A\subseteq\Omega
\]

\[
x\in States(A)\Rightarrow x\in\Omega
\]

Đây gần như là hệ quả định nghĩa của Total Reality.

## BL-A02 — Tiên đề Biểu diễn Nội sinh

Nếu A sinh representation \(r\), sự kiện sinh và trạng thái mang representation đó nằm trong \(\Omega\):

\[
Generate_A(r)\Rightarrow E_R(r)=1
\]

Trong đó \(E_R\) là representational existence.

A02 **không** nói referent được actualize vật lý.

## BL-A03 — Tiên đề Không-đồng-nhất Biên

Không mặc định biên nhận thức của một embedded observer là biên của Tổng Thực Tại:

\[
Boundary(K_A)\not\equiv Boundary(\Omega)
\]

Đây là nguyên tắc phương pháp: equality cần proof, không được gán từ thiếu dữ liệu.

## BL-A04 — Tiên đề Bản thể Mở

Tập primitive/category hiện được một chủ thể biết không được mặc định là tập primitive/category cuối cùng của \(\Omega\):

\[
P_{known}\not\equiv P_{final}
\]

trừ khi có một closure argument đủ mạnh.

## BL-A05 — Tiên đề Khả đạt Động

Đối với agent có thể chế tạo công cụ, reachable space phụ thuộc repertoire và có thể thay đổi:

\[
\mathcal R_A(t)=Reach(x_t,U_t)
\]

\[
U_t\neq U_{t+1}\Rightarrow \mathcal R_A(t)\text{ có thể thay đổi}
\]

Không khẳng định mọi expansion đều monotonic trong đời thực; tool có thể mất, tài nguyên có thể suy giảm. Monotonicity chỉ đúng khi các capability cũ được bảo toàn.

## BL-A06 — Tiên đề Bộ lọc Quan sát

Dữ liệu mà A gọi là “thực tại đã biết” là đầu ra của chuỗi bộ lọc:

\[
D_A=F_{cognition}\circ F_{detector}\circ F_{access}\circ F_{causal}\circ F_{stability}(\Omega)
\]

Vì vậy không mặc định distribution của dữ liệu quan sát bằng distribution toàn ontology.

## BL-A07 — Tiên đề Bảo tồn Phát kiến

Trong governance của BLOK, một hypothesis có thể được bảo tồn mà chưa được endorsement:

\[
Preserve(H)\not\Rightarrow Accept(H)
\]

Chi phí lưu giữ thấp làm việc tách preservation khỏi validation trở thành một chiến lược tối ưu đáng nghiên cứu.

## BL-A08 — Tiên đề Đồng cấp Phản biện

Một refutation có sức nặng phải tác động vào biến quyết định truth-status của claim mục tiêu. Status, công cụ hay môi trường làm việc của tác giả không tự động là biến truth-making.

A08 không cấm critique về phương pháp hay công cụ; nếu công cụ tạo lỗi cụ thể, critique đó hoàn toàn admissible.

## BL-A09 — Tiên đề Plenitude BL∞ (tùy chọn, phiên bản mạnh)

Đây là **extension**, không phải lõi bắt buộc:

\[
Cons_L(h)\Rightarrow \exists W\subseteq\Omega:W\models_L h
\]

Nếu chấp nhận A09, mọi cấu trúc nhất quán theo logic L được hiện thực hóa ở ít nhất một miền của Total Reality. Origin build không tuyên bố A09 đã được vật lý chứng minh.

## BL-A10 — Hyper-Plenitude (speculative)

Trong extension phi cổ điển, một số “impossible structures” theo classical logic có thể được xét trong logic paraconsistent hoặc impossible-world semantics. Đây là vùng nghiên cứu mở và phải luôn gắn nhãn speculative.
