...
function Test-UnauthenticatedExecute([int]$Port) {
  try {
    ...
    return [pscustomobject]@{ pass = $false; status = 'unexpected-success' }
  }
  catch {
    $response = $_.Exception.Response
    if ($response) {
      try {
        if ([int]$response.StatusCode -eq 401) { return [pscustomobject]@{ pass = $true; status = '401' } }
      } catch {}
    }
    return [pscustomobject]@{ pass = $false; status = $_.Exception.Message }
  }
}
...
try {
  ...
  Add-Check 'Firewall-Readable' $false $_.Exception.Message 'P0'
}
...
catch { Add-Check 'RemoteRegistry-Readable' $false $_.Exception.Message 'P1' }
...
if (Test-Path $ConfigPath) {
  try {
    ...
    Add-Check 'Node-Config-State' ([string]$config.state -eq 'STAGED_LOCAL_ONLY') "state=$($config.state)" 'P0'
    Add-Check 'Node-Remote-Routable-False' (-not [bool]$config.remoteRoutable) "remoteRoutable=$($config.remoteRoutable)" 'P0'
    Add-Check 'Node-Host-Loopback' ([string]$config.workerHost -eq '127.0.0.1') "workerHost=$($config.workerHost)" 'P0'
    $WorkerPort = [int]$config.workerPort
  }
  catch { Add-Check 'Node-Config-Readable' $false $_.Exception.Message 'P0' }
}
else { Add-Check 'Node-Config-Present' $false "$ConfigPath missing" 'P0' }
...
catch { Add-Check 'Startup-Task-Readable' $false $_.Exception.Message 'P1' }
...
$listenerStrings = @($listeners | ForEach-Object { "$($_.LocalAddress):$($_.LocalPort)" })
Add-Check 'Worker-No-NonLoopback-Listener' ($unsafe.Count -eq 0) ("listeners=" + ($listenerStrings -join ',')) 'P0'
...
try {
  ...
  $capDiff = @(Compare-Object $allowed $actual)
  Add-Check 'Worker-Capability-Allowlist' ($capDiff.Count -eq 0) ("capabilities=" + ($actual -join ',')) 'P0'
...
catch { Add-Check 'Worker-Local-Health' $false $_.Exception.Message 'P0' }

$p0Failures = @($script:checks | Where-Object { $_.severity -eq 'P0' -and -not $_.pass })
$p1Failures = @($script:checks | Where-Object { $_.severity -eq 'P1' -and -not $_.pass })
$verdict = 'FAIL_CLOSED'
if ($p0Failures.Count -eq 0 -and $healthOk) { $verdict = 'READY_LOCAL_ONLY' }
elseif ($p0Failures.Count -eq 0) { $verdict = 'HARDENED_RUNTIME_NOT_READY' }
...
$result = [pscustomobject]@{
  ...
  summary = [pscustomobject]@{
    p0Failures = $p0Failures.Count
    p1Failures = $p1Failures.Count
...
Write-Host "DEUS physical-node verdict: $verdict" -ForegroundColor $(if ($verdict -eq 'READY_LOCAL_ONLY') { 'Green' } elseif ($verdict -eq 'HARDENED_RUNTIME_NOT_READY') { 'Yellow' } else { 'Red' })
