...
export function searchDocumentContentHash(doc) {
  ...
  return sha256Json({
    title: doc.title ?? '', text: doc.text ?? '', source: doc.source ?? null,
    tenantId: doc.tenantId ?? 'default', dataClass: doc.dataClass ?? 'public',
...
export class HybridSearchFabric {
  ...
  addDocument(doc) {
    ...
    const normalized = { ...structuredClone(doc), tenantId: doc.tenantId ?? 'default', dataClass: doc.dataClass ?? 'public' };
    ...
    const stored = { ...normalized, tokens, tf, vector: hashedVector(tokens, this.vectorDims), trust: clamp01(normalized.trust ?? 0.5), contentHash: searchDocumentContentHash(normalized) };
  ...
  removeDocument(id) {
    const old = this.docs.get(id); if (!old) return false;
    for (const token of old.tf.keys()) { const next = (this.df.get(token) ?? 1) - 1; if (next <= 0) this.df.delete(token); else this.df.set(token, next); }
    this.totalLength -= old.tokens.length; this.docs.delete(id); this.graph.delete(id);
    for (const edges of this.graph.values()) edges.delete(id);
    return true;
  }

  getDocument(id) { const doc = this.docs.get(id); return doc ? publicDoc(doc) : null; }

  search(query, { limit = 10, seedIds = [], lexicalWeight = 0.5, semanticWeight = 0.3, graphWeight = 0.1, trustWeight = 0.1, freshnessHalfLifeDays = 365, allowedDataClasses = ['public','internal','private'], tenantId = null } = {}) {
    ...
    const graphBoost = this.#graphBoost(seedIds, visibleIds);
  ...
  #graphBoost(seedIds, visibleIds) {
    const scores = new Map();
    for (const seed of seedIds) {
      if (!visibleIds.has(seed)) continue;
      scores.set(seed, Math.max(scores.get(seed) ?? 0, 1));
      for (const [target, weight] of this.graph.get(seed) ?? []) {
        if (!visibleIds.has(target)) continue;
        scores.set(target, Math.max(scores.get(target) ?? 0, Math.min(1, weight)));
      }
    }
    return scores;
  }
...
function documentFrequency(docs) { const df = new Map(); for (const doc of docs) for (const token of doc.tf.keys()) df.set(token, (df.get(token) ?? 0) + 1); return df; }
function bm25(queryTokens, doc, df, nDocs, avgdl, k1 = 1.2, b = 0.75) { let score = 0; for (const term of new Set(queryTokens)) { const freq = doc.tf.get(term) ?? 0; if (!freq) continue; const dfi = df.get(term) ?? 0; const idf = Math.log(1 + (nDocs - dfi + 0.5) / (dfi + 0.5)); const denom = freq + k1 * (1 - b + b * doc.tokens.length / Math.max(1, avgdl)); score += idf * (freq * (k1 + 1)) / denom; } return score; }
function normalizeRelations(relations) { return relations.map((rel) => ({ target: String(rel.target), weight: Number(rel.weight ?? 1) })).sort((a, b) => a.target.localeCompare(b.target) || a.weight - b.weight); }
