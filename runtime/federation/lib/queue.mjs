...
export class MemoryLeaseQueue {
  ...
  enqueue(task, { idempotencyKey = null, priority = 0, maxAttempts = this.maxAttempts, now = Date.now() } = {}) {
    validateTask(task);
    const key = idempotencyKey ?? task.idempotencyKey ?? sha256Json({ capability: task.capability, payload: task.payload ?? null, dataClass: task.dataClass ?? 'public' });
  ...
  claim(workerId, capabilities = [], { leaseMs = this.defaultLeaseMs, now = Date.now() } = {}) {
    ...
    const candidates = [...this.jobs.values()].filter((j) => j.state === 'pending' && j.availableAt <= now && capabilities.includes(j.task.capability)).sort((a, b) => b.priority - a.priority || a.availableAt - b.availableAt || a.createdAt.localeCompare(b.createdAt));
    const job = candidates[0]; if (!job) return null;
    const token = randomUUID(); job.state = 'running'; job.attempts += 1; job.lease = { workerId, token, expiresAt: now + leaseMs, lastHeartbeatAt: now }; return this.get(job.id);
  }
