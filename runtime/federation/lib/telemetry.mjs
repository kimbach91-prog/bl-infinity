...
export class TelemetryBook {
  constructor({ alpha = 0.2, latencyWindow = 64 } = {}) { this.alpha = alpha; this.latencyWindow = latencyWindow; this.map = new Map(); }
    ...
    if (!this.map.has(providerId)) this.map.set(providerId, {
      availability: seed.availability ?? 1,
      trust: seed.trust ?? 0.5,
      p95LatencyMs: seed.p95LatencyMs ?? 1000,
      costPerUnitUsd: seed.costPerUnitUsd ?? 0,
      energyPerUnitJoules: nonNegativeFiniteOrNull(seed.energyPerUnitJoules),
      inFlight: seed.inFlight ?? 0,
      successes: 0,
      failures: 0,
      latencies: [],
      lastSeenAt: null,
  ...
  success(providerId, { latencyMs, costUsd = 0, energyJoules = null, workUnits = 1, energyPerUnitJoules = null } = {}) {
    ...
    const directEnergy = nonNegativeFiniteOrNull(energyPerUnitJoules);
    const totalEnergy = nonNegativeFiniteOrNull(energyJoules);
    const units = Number(workUnits);
    const energySample = directEnergy ?? (totalEnergy != null && Number.isFinite(units) && units > 0 ? totalEnergy / units : null);
    if (energySample != null) t.energyPerUnitJoules = ewma(t.energyPerUnitJoules, energySample, this.alpha);
  ...
  updateEnergy(providerId, { energyJoules = null, workUnits = 1, energyPerUnitJoules = null } = {}) {
    const t = this.ensure(providerId);
    const directEnergy = nonNegativeFiniteOrNull(energyPerUnitJoules);
    const totalEnergy = nonNegativeFiniteOrNull(energyJoules);
    const units = Number(workUnits);
    const sample = directEnergy ?? (totalEnergy != null && Number.isFinite(units) && units > 0 ? totalEnergy / units : null);
    if (sample == null) throw new Error('measured energy sample is required');
    t.energyPerUnitJoules = ewma(t.energyPerUnitJoules, sample, this.alpha);
    t.lastSeenAt = new Date().toISOString();
