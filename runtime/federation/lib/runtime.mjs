...
export function createFederationRuntime({
  ...
  valuePolicy = null,
  computeTreasury = null,
  computeTreasuryConfig = {},
  ...
  const circuit = new CircuitBreakerBook();
  const seeded = providers.map((provider) => {
    telemetry.ensure(provider.id, provider.telemetry ?? {});
    return { ...structuredClone(provider), telemetry: telemetry.snapshot(provider.id) };
  });
  const registry = new ProviderRegistry(seeded, { manifestVerifier });
  const executor = new FederationExecutor({
    registry,
    telemetry,
    circuit,
    audit: effectiveAudit,
    adapters: {
      local: new LocalAdapter(localHandlers),
      'http-worker': new HttpWorkerAdapter(),
