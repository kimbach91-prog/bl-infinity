...
export function createFederationRuntime({ providers = [], localHandlers = {}, manifestVerifier = null, audit = new MemoryAuditLog(), budgetConfig = {} } = {}) {
  ...
  const executor = new FederationExecutor({ registry, telemetry, circuit, audit, adapters: { local: new LocalAdapter(localHandlers), 'http-worker': new HttpWorkerAdapter(), 'cloudflare-worker': new HttpWorkerAdapter(), 'gcp-cloud-run': new HttpWorkerAdapter(), 'vercel-function': new HttpWorkerAdapter() } });
  const orchestrator = new FederationOrchestrator({ registry, executor, audit, budget: new BudgetGovernor(budgetConfig) });
  return { registry, telemetry, circuit, executor, orchestrator, audit };
}
