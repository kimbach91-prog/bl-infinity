...
export function createWorkerServer({ handlers = safeDefaultHandlers, sharedSecret = process.env.BL_FEDERATION_SHARED_SECRET, requireAuth = true, maxConcurrency = Number(process.env.BL_WORKER_MAX_CONCURRENCY || 2), maxBodyBytes = Number(process.env.BL_WORKER_MAX_BODY_BYTES || 1_048_576) } = {}) {
  ...
  return http.createServer(async (req, res) => {
    ...
    let body;
    try { body = await readBody(req, maxBodyBytes); } catch (error) { return send(res, error.code === 'BODY_TOO_LARGE' ? 413 : 400, { error: error.message }); }
    if (requireAuth) {
      if (!sharedSecret) return send(res, 503, { error: 'worker auth is not configured' });
      const verdict = verifyWorkerEnvelope(sharedSecret, req.headers, body, { replayGuard });
      if (!verdict.ok) return send(res, 401, { error: verdict.reason });
    }
    let envelope;
    try { envelope = JSON.parse(body || '{}'); } catch { return send(res, 400, { error: 'invalid JSON' }); }
    const task = envelope.task;
    if (!task?.id || !task?.capability) return send(res, 400, { error: 'task.id and task.capability are required' });
    const handler = handlerMap.get(task.capability);
    if (!handler) return send(res, 403, { error: 'capability not installed' });
    if (task.deadlineAt && Date.parse(task.deadlineAt) <= Date.now()) return send(res, 408, { error: 'task deadline expired' });
    inFlight += 1;
    try { return send(res, 200, { ok: true, taskId: task.id, result: await handler(structuredClone(task.payload), { task: structuredClone(task) }) }); }
...
export function startWorkerFromEnv() {
  ...
  server.listen(port, host, () => console.log(`BL federation worker listening on http://${host}:${port}`));
  return server;
}
