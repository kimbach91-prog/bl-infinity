...
const runtime = createFederationRuntime({ providers, localHandlers: safeDefaultHandlers, manifestVerifier });
const search = new HybridSearchFabric();

const server = http.createServer(async (req, res) => {
  setCommonHeaders(res);
  try {
    if (req.method === 'GET' && req.url === '/health') return send(res, 200, { ok: true, service: 'bl-compute-federation', version: '0.2.0', providers: runtime.registry.list().length, search: search.stats(), signedManifestsRequired: requireSignedManifests });
...
async function loadProviders() {
  ...
  const file = process.env.BL_PROVIDER_FILE || new URL('./config/providers.example.json', import.meta.url); return JSON.parse(await readFile(file, 'utf8'));
}
function parseJsonEnv(name, fallback) { return process.env[name] ? JSON.parse(process.env[name]) : fallback; }
function authorizedIfConfigured(req) { return controlToken ? bearer(req) === controlToken : true; }
function requireControl(req) { return Boolean(controlToken && bearer(req) === controlToken); }
