...
def c25_apply_mpfb_enhanced_skin(material):
    ...
    if C41_NATIVE_EYE_SCALP_FLOW:
        settings.update({
            "Clearcoat":0.003,
            "Clearcoat Roughness":0.56,
            "Pore detail":4.4,
            "Pore distortion":1.10,
            "Pore scale":2150.0,
            "Pore strength":0.68,
            "Roughness":0.615,
            "colorMixInStrength":0.003,
            "SSS strength":0.024,
            "SSS radius scale":0.036,
        })
...
def c25_apply_mpfb_procedural_eyes(material):
    ...
    if C41_NATIVE_EYE_SCALP_FLOW:
        settings.update({
            "EyeWhiteColor":(0.86,0.83,0.80,1.0),
            "IrisBumpStrength":0.36,
            "IrisToEyeWhiteRelation":0.445,
            "PupilSize":0.315,
            "OuterLayerRoughness":0.012,
            "InnerLayerRoughness":0.075,
        })
    c25_set_group_values(group,settings)
    values=c25_group_values(group)
...
C41_NATIVE_EYE_SCALP_FLOW=os.environ.get("DIGE_C41_NATIVE_EYE_SCALP_FLOW","0").strip()=="1"
C42_EYE_GEOMETRY_SKIN=os.environ.get("DIGE_C42_EYE_GEOMETRY_SKIN","0").strip()=="1"
C43_HYBRID_REALISM=os.environ.get("DIGE_C43_HYBRID_REALISM","0").strip()=="1"
C44_UV_TRANSFER_SKIN=os.environ.get("DIGE_C44_UV_TRANSFER_SKIN","0").strip()=="1"
C45_BARYCENTRIC_UV_SKIN=os.environ.get("DIGE_C45_BARYCENTRIC_UV_SKIN","0").strip()=="1"
...
def c28_gnm_skin_material():
    ...
    if C31_GNM_FACE_APPEARANCE:
        ...
        if hasattr(bs,"subsurface_method"):
            bs.subsurface_method='RANDOM_WALK_SKIN'
        if C42_EYE_GEOMETRY_SKIN:
            set_input(bs,"Specular IOR Level",.26)
            set_input(bs,"Subsurface Weight",.012)
            set_input(bs,"Subsurface Scale",.00042)
            set_input(bs,"Subsurface Radius",(1.0,.34,.14))
            set_input(bs,"Coat Weight",0.0)
            set_input(bs,"Coat Roughness",.62)
        if C43_HYBRID_REALISM:
            set_input(bs,"Specular IOR Level",.24)
            set_input(bs,"Subsurface Weight",.009)
            set_input(bs,"Subsurface Scale",.00036)
            set_input(bs,"Subsurface Radius",(1.0,.32,.12))
            set_input(bs,"Coat Weight",0.0)
            set_input(bs,"Coat Roughness",.66)
        ...
        base_ramp.color_ramp.elements[0].color=((0.205,0.135,0.120,1) if C42_EYE_GEOMETRY_SKIN else ((0.220,0.125,0.105,1) if C36_CANONICAL_APPEARANCE else ((0.205,0.073,0.048,1) if C32_GNM_DERMAL_HAIR else (0.245,0.082,0.042,1))))
        base_ramp.color_ramp.elements[1].position=.82
        base_ramp.color_ramp.elements[1].color=((0.350,0.245,0.215,1) if C42_EYE_GEOMETRY_SKIN else ((0.365,0.225,0.190,1) if C36_CANONICAL_APPEARANCE else ((0.395,0.176,0.118,1) if C32_GNM_DERMAL_HAIR else (0.445,0.190,0.102,1))))
        ...
        lip_color.outputs[0].default_value=(0.255,0.082,0.080,1) if C42_EYE_GEOMETRY_SKIN else ((0.30,0.060,0.065,1) if C36_CANONICAL_APPEARANCE else (0.42,0.075,0.068,1))
        ...
        blush_color.outputs[0].default_value=(0.285,0.120,0.110,1) if C42_EYE_GEOMETRY_SKIN else ((0.34,0.105,0.095,1) if C36_CANONICAL_APPEARANCE else (0.46,0.135,0.088,1))
        ...
        rough_map.inputs["From Min"].default_value=0.0
        rough_map.inputs["From Max"].default_value=1.0
        rough_map.inputs["To Min"].default_value=.50 if C42_EYE_GEOMETRY_SKIN else (.38 if C36_CANONICAL_APPEARANCE else (.46 if C32_GNM_DERMAL_HAIR else .44))
        rough_map.inputs["To Max"].default_value=.68 if C42_EYE_GEOMETRY_SKIN else (.57 if C36_CANONICAL_APPEARANCE else (.64 if C32_GNM_DERMAL_HAIR else .60))
        ...
        pore=nt.nodes.new("ShaderNodeTexNoise")
        pore.inputs["Scale"].default_value=1180.0 if C42_EYE_GEOMETRY_SKIN else (760.0 if C36_CANONICAL_APPEARANCE else (2400.0 if C32_GNM_DERMAL_HAIR else 1900.0))
        ...
        micro=nt.nodes.new("ShaderNodeTexNoise")
        micro.inputs["Scale"].default_value=3600.0 if C42_EYE_GEOMETRY_SKIN else (2400.0 if C36_CANONICAL_APPEARANCE else (9000.0 if C32_GNM_DERMAL_HAIR else 6200.0))
        ...
        bump=nt.nodes.new("ShaderNodeBump")
        bump.inputs["Strength"].default_value=.46 if C42_EYE_GEOMETRY_SKIN else (.24 if C36_CANONICAL_APPEARANCE else (.34 if C32_GNM_DERMAL_HAIR else .24))
        bump.inputs["Distance"].default_value=.000060 if C42_EYE_GEOMETRY_SKIN else (.000038 if C36_CANONICAL_APPEARANCE else (.000052 if C32_GNM_DERMAL_HAIR else .000070))
        nt.links.new(add.outputs[0],bump.inputs["Height"])
        nt.links.new(bump.outputs["Normal"],bs.inputs["Normal"])
        nt.links.new(bs.outputs[0],out.inputs["Surface"])
        return m,{
            "model":"C43_GNM_HYBRID_REALISM_PBR_SKIN_V1" if C43_HYBRID_REALISM else ("C42_GNM_VISIBLE_SCALE_PBR_SKIN_V1" if C42_EYE_GEOMETRY_SKIN else ("C36_CANONICAL_PBR_SKIN_V1" if C36_CANONICAL_APPEARANCE else ("C32_GNM_DERMAL_MULTISCALE_SKIN" if C32_GNM_DERMAL_HAIR else "C31_GNM_REGIONAL_RANDOM_WALK_SKIN"))),
            ...
            "roughness_range":([0.50,0.68] if C42_EYE_GEOMETRY_SKIN else ([0.38,0.57] if C36_CANONICAL_APPEARANCE else ([0.46,0.64] if C32_GNM_DERMAL_HAIR else [0.44,0.60]))),
            "subsurface_weight":0.012 if C42_EYE_GEOMETRY_SKIN else (0.030 if C36_CANONICAL_APPEARANCE else (0.045 if C32_GNM_DERMAL_HAIR else 0.070)),
            "subsurface_scale":0.00090 if C36_CANONICAL_APPEARANCE else (0.00064 if C32_GNM_DERMAL_HAIR else 0.00082),
            "pore_scale":1180.0 if C42_EYE_GEOMETRY_SKIN else (760.0 if C36_CANONICAL_APPEARANCE else (2400.0 if C32_GNM_DERMAL_HAIR else 1900.0)),
            "micro_scale":3600.0 if C42_EYE_GEOMETRY_SKIN else (2400.0 if C36_CANONICAL_APPEARANCE else (9000.0 if C32_GNM_DERMAL_HAIR else 6200.0)),
...
if C28_GNM_HEAD:
    ...
    if C42_EYE_GEOMETRY_SKIN:
        ...
        c28_eye_contract={
            "enabled":True,
            "material_model":"C42_GNM_SCLERA_PLUS_SURFACE_GEOMETRY_IRIS",
            "integration":"GNM_NATIVE_EYEBALL_MESH_PLUS_SURFACE_IRIS_PUPIL_NO_UV_DEPENDENCE",
            "c42_eye_geometry":True,
        }
    elif C29_GNM_PRESENTATION and not C41_NATIVE_EYE_SCALP_FLOW:
        gnm_eye_mat=sclera
        c28_eye_contract={
            "enabled":True,
        ...
        if C41_NATIVE_EYE_SCALP_FLOW:
            c28_eye_contract=dict(c28_eye_contract)
            c28_eye_contract["c41_native_eye_mesh"]=True
            c28_eye_contract["integration"]="GNM_NATIVE_EYE_MESH_PLUS_MPFB2_PROCEDURAL_EYE_NO_CYLINDER_OVERLAY"
        if not C25_MATURE_STACK:
            gnm_eye_mat=eye_mat
    ...
    c44_uv_transfer_contract={"enabled":False}
    if C44_UV_TRANSFER_SKIN:
        skin_obj=c28_gnm_objects.get("skin")
        if skin_obj is None:
            raise RuntimeError("C44 missing GNM skin component")
        if not body.data.uv_layers or body.data.uv_layers.active is None:
            raise RuntimeError("C44 source MakeHuman body has no active UV layer")
        src_uv=body.data.uv_layers.active.data
        body.data.calc_loop_triangles()
        src_world=[body.matrix_world @ v.co for v in body.data.vertices]
        tri_defs=[]
        tri_loop_ids=[]
        for tri in body.data.loop_triangles:
            tri_defs.append([int(x) for x in tri.vertices])
            tri_loop_ids.append([int(x) for x in tri.loops])
        if not tri_defs:
            raise RuntimeError("C45 source body has no loop triangles")
        bvh=BVHTree.FromPolygons(src_world,tri_defs,all_triangles=True)
        uv_name="C45_BARYCENTRIC_MAKEHUMAN_UV"
        dst_layer=skin_obj.data.uv_layers.get(uv_name) or skin_obj.data.uv_layers.new(name=uv_name)
        distances=[]
        invalid=0
        for poly in skin_obj.data.polygons:
            for li in poly.loop_indices:
                vi=skin_obj.data.loops[li].vertex_index
                wp=skin_obj.matrix_world @ skin_obj.data.vertices[vi].co
                nearest=bvh.find_nearest(wp)
                if nearest is None:
                    invalid+=1
                    dst_layer.data[li].uv=(.5,.5)
                    continue
                loc,_normal,tri_idx,dist=nearest
                tri=tri_defs[int(tri_idx)]
                loops=tri_loop_ids[int(tri_idx)]
                p0,p1,p2=(src_world[tri[0]],src_world[tri[1]],src_world[tri[2]])
                uv0,uv1,uv2=(src_uv[loops[0]].uv,src_uv[loops[1]].uv,src_uv[loops[2]].uv)
                ...
                dst_layer.data[li].uv=(float(uv3.x),float(uv3.y))
                distances.append(float(dist))
        if invalid:
            raise RuntimeError(f"C45 barycentric UV nearest-surface misses: {invalid}")
        if not distances:
            raise RuntimeError("C45 barycentric UV transfer produced no loop distances")

        albedo_path=Path(SKIN_ALBEDO_PATH)
        if not albedo_path.is_absolute():
            albedo_path=ROOT/albedo_path
        if not albedo_path.exists():
            raise RuntimeError(f"C44 source albedo missing: {albedo_path}")
        albedo_sha=sha(albedo_path)
        if SKIN_ALBEDO_EXPECTED_SHA256 and albedo_sha.lower()!=SKIN_ALBEDO_EXPECTED_SHA256:
            raise RuntimeError(f"C44 albedo hash drift expected={SKIN_ALBEDO_EXPECTED_SHA256} got={albedo_sha}")

        c44mat=bpy.data.materials.new("DIGE_C45_GNM_BARYCENTRIC_SKIN")
        c44mat.use_nodes=True
        nt=c44mat.node_tree
        bs=nt.nodes.get("Principled BSDF")
        set_input(bs,"IOR",1.42)
        set_input(bs,"Specular IOR Level",.24)
        set_input(bs,"Roughness",.56)
        set_input(bs,"Subsurface Weight",.010)
        set_input(bs,"Subsurface Scale",.00040)
        set_input(bs,"Subsurface Radius",(1.0,.34,.14))
        set_input(bs,"Coat Weight",0.0)
        set_input(bs,"Coat Roughness",.68)
        if hasattr(bs,"subsurface_method"):
            bs.subsurface_method='RANDOM_WALK_SKIN'

        uvnode=nt.nodes.new("ShaderNodeUVMap")
        uvnode.uv_map=uv_name
        img=bpy.data.images.load(str(albedo_path),check_existing=True)
        try: img.colorspace_settings.name='sRGB'
        except Exception: pass
        teximg=nt.nodes.new("ShaderNodeTexImage")
        teximg.image=img
        teximg.interpolation='Linear'
        nt.links.new(uvnode.outputs["UV"],teximg.inputs["Vector"])
        grade=nt.nodes.new("ShaderNodeHueSaturation")
        grade.inputs["Saturation"].default_value=.88
        grade.inputs["Value"].default_value=.78
        nt.links.new(teximg.outputs["Color"],grade.inputs["Color"])

        # Small broad variation preserves scan-like pigmentation without washing out the source texture.
        broad=nt.nodes.new("ShaderNodeTexNoise")
        broad.inputs["Scale"].default_value=7.0
        broad.inputs["Detail"].default_value=3.0
        broad.inputs["Roughness"].default_value=.58
        nt.links.new(uvnode.outputs["UV"],broad.inputs["Vector"])
        bramp=nt.nodes.new("ShaderNodeValToRGB")
        bramp.color_ramp.elements[0].color=(.86,.82,.80,1)
        bramp.color_ramp.elements[1].color=(1.04,1.00,.98,1)
        nt.links.new(broad.outputs["Fac"],bramp.inputs["Fac"])
        cmix=nt.nodes.new("ShaderNodeMixRGB")
        cmix.blend_type='MULTIPLY'
        cmix.inputs["Fac"].default_value=.18
        nt.links.new(grade.outputs["Color"],cmix.inputs[1])
        nt.links.new(bramp.outputs["Color"],cmix.inputs[2])
        nt.links.new(cmix.outputs["Color"],bs.inputs["Base Color"])

        rough=nt.nodes.new("ShaderNodeTexNoise")
        rough.inputs["Scale"].default_value=72.0
        rough.inputs["Detail"].default_value=4.0
        rough.inputs["Roughness"].default_value=.64
        nt.links.new(uvnode.outputs["UV"],rough.inputs["Vector"])
        rmap=nt.nodes.new("ShaderNodeMapRange")
        rmap.inputs["From Min"].default_value=0.0
        rmap.inputs["From Max"].default_value=1.0
        rmap.inputs["To Min"].default_value=.49
        rmap.inputs["To Max"].default_value=.67
        nt.links.new(rough.outputs["Fac"],rmap.inputs["Value"])
        nt.links.new(rmap.outputs["Result"],bs.inputs["Roughness"])

        pore=nt.nodes.new("ShaderNodeTexNoise")
        pore.inputs["Scale"].default_value=1200.0
        pore.inputs["Detail"].default_value=4.0
        pore.inputs["Roughness"].default_value=.68
        nt.links.new(uvnode.outputs["UV"],pore.inputs["Vector"])
        micro=nt.nodes.new("ShaderNodeTexNoise")
        micro.inputs["Scale"].default_value=3600.0
        micro.inputs["Detail"].default_value=2.0
        micro.inputs["Roughness"].default_value=.60
        nt.links.new(uvnode.outputs["UV"],micro.inputs["Vector"])
        p1=nt.nodes.new("ShaderNodeMath"); p1.operation='MULTIPLY'; p1.inputs[1].default_value=.65
        p2=nt.nodes.new("ShaderNodeMath"); p2.operation='MULTIPLY'; p2.inputs[1].default_value=.35
        add=nt.nodes.new("ShaderNodeMath"); add.operation='ADD'
        nt.links.new(pore.outputs["Fac"],p1.inputs[0]); nt.links.new(micro.outputs["Fac"],p2.inputs[0])
        nt.links.new(p1.outputs[0],add.inputs[0]); nt.links.new(p2.outputs[0],add.inputs[1])
        bump=nt.nodes.new("ShaderNodeBump")
        ...
        skin_obj.data.materials.append(c44mat)
        sd=sorted(distances)
        p95=sd[min(len(sd)-1,int(len(sd)*.95))]
        c44_uv_transfer_contract={
            "enabled":True,
            "method":"NEAREST_SURFACE_TRIANGLE_BARYCENTRIC_UV",
            "source_object":body.name,
            "source_uv_layer":body.data.uv_layers.active.name,
            "target_uv_layer":uv_name,
            "loops_transferred":len(distances),
            "mean_distance_m":sum(distances)/len(distances),
            "p95_distance_m":p95,
            "max_distance_m":max(distances),
            "albedo_file":albedo_path.name,
            "albedo_sha256":albedo_sha,
            "material":"DIGE_C45_GNM_BARYCENTRIC_SKIN",
            "truth_boundary":"BARYCENTRIC_SURFACE_UV_TRANSFER_FROM_EXISTING_CC0_MAKEHUMAN_TEXTURE__NO_REFERENCE_IMAGE_PIXELS"
        }
        c28_skin_contract={
            "model":"C45_GNM_BARYCENTRIC_UV_CC0_SKIN_V1",
            "source_albedo_sha256":albedo_sha,
            "uv_transfer":c44_uv_transfer_contract,
            "roughness_range":[.49,.67],
            "subsurface_weight":.010,
            "subsurface_scale":.00040,
            "pore_scale":1200.0,
            "micro_scale":3600.0,
    ...
    if C29_GNM_PRESENTATION and not C41_NATIVE_EYE_SCALP_FLOW:
        for comp,pts in (("left_eye",transformed_landmarks[36:42]),("right_eye",transformed_landmarks[42:48])):
            eye_mesh=c28_gnm_objects.get(comp)
            if eye_mesh is None:
    ...
    if C42_EYE_GEOMETRY_SKIN:
        # C42: keep the real GNM eyeball as sclera/cornea substrate, then place only
        # a thin iris/limbal/pupil surface stack on the actual front plane. No
        # protruding cornea cylinder and no UV-dependent procedural iris.
        for comp,pts in (("left_eye",transformed_landmarks[36:42]),("right_eye",transformed_landmarks[42:48])):
            eye_mesh=c28_gnm_objects.get(comp)
            if eye_mesh is None:
                raise RuntimeError(f"C42 missing GNM eye component: {comp}")
            ec=sum(pts,Vector())/len(pts)
            front_y=max(float(v.co.y) for v in eye_mesh.data.vertices)
            eye_w=max(p.x for p in pts)-min(p.x for p in pts)
            iris_r=max(.00320,min(.00382,eye_w*(.180 if C43_HYBRID_REALISM else .205)))
            pupil_r=iris_r*(.335 if C43_HYBRID_REALISM else .365)
            iy=front_y+(.000018 if C43_HYBRID_REALISM else .000035)
            cylinder(f"DIGE_C42_{comp.upper()}_LIMBAL",(ec.x,iy,ec.z),iris_r*1.045,.000018 if C43_HYBRID_REALISM else .000026,iris_ring)
            cylinder(f"DIGE_C42_{comp.upper()}_IRIS",(ec.x,iy+.000012,ec.z),iris_r,.000016 if C43_HYBRID_REALISM else .000024,iris)
            cylinder(f"DIGE_C42_{comp.upper()}_PUPIL",(ec.x,iy+.000024,ec.z),pupil_r,.000014 if C43_HYBRID_REALISM else .000022,black)
            c42_iris_count+=1
    ...
    curve_object("DIGE_C28_GNM_BROW_FIBERS",brow_fibers,.000125 if C40_HYPERREAL_REPAIR else (.000095 if C36_CANONICAL_APPEARANCE else (.000082 if C31_GNM_FACE_APPEARANCE else .000060)),facial_hair_mat)
    ...
    c28_gnm={
        ...
        "c42_surface_iris_count":c42_iris_count,
...
if C39_HHIR_HYPERREAL:
    if not C28_GNM_HEAD:
        raise RuntimeError("C40 HHIR hyperreal requires GNM head geometry")
    try:
        hair_groom.hide_render=True
        hair_groom.hide_set(True)
    except Exception:
        pass
    hrng=random.Random(20264040)
    hairline=float(c28_gnm["hairline_target_z"])
    bbox_min=Vector(c28_gnm["aligned_bbox_min"])
    bbox_max=Vector(c28_gnm["aligned_bbox_max"])
    eye_center=sum(transformed_landmarks[36:48],Vector())/12
    bun_center=Vector((0.0, eye_center.y-.066, min(float(bbox_max.z)+.002, hairline+.078)))

    # C40: root the visible updo directly on the actual GNM scalp surface.
    ...
    for v in groom_surface.data.vertices:
        ...
        if p.z < hairline-.003 or p.z > float(bbox_max.z)+.003:
            continue
        if abs(p.x) > .128 or p.y > eye_center.y+.046:
            continue
        if p.y > eye_center.y+.010 and p.z < hairline+.014:
            continue
        if n.y > .78 and p.z < hairline+.028:
            continue
        scalp.append((v.index,p,n))
    if len(scalp) < 500:
        raise RuntimeError(f"C40 scalp coverage too sparse: {len(scalp)}")
    scalp.sort(key=lambda q:q[0])
    target_primary=min(2400,len(scalp))
    roots=[scalp[min(len(scalp)-1,int(i*len(scalp)/target_primary))] for i in range(target_primary)]
    if C43_HYBRID_REALISM:
        back_roots=[q for q in roots if not (q[1].y > eye_center.y-.012 and q[1].z < hairline+.034)]
        front_roots=[q for q in roots if (q[1].y > eye_center.y-.012 and q[1].z < hairline+.034)]
        front_sparse=front_roots[::max(1,len(front_roots)//72)] if front_roots else []
        roots=back_roots+front_sparse

    primary=[]
    undercoat=[]
    inv_surface=groom_surface.matrix_world.inverted()
    surf_world=groom_surface.matrix_world
    def c41_project(world_p,fallback_n):
        hit,loc_local,n_local,_poly=groom_surface.closest_point_on_mesh(inv_surface @ world_p)
        if not hit:
            return world_p,fallback_n
        wp=surf_world @ loc_local
        wn=(surf_rot @ n_local).normalized()
        return wp+wn*.00055,wn

    for vi,root,n in roots:
        sx=max(-1.0,min(1.0,root.x/.125))
        sy=max(-1.0,min(1.0,(root.y-eye_center.y)/.11))
        target=bun_center+Vector((.030*sx,.015*sy,-.008+.010*(1.0-abs(sx))))
        d=target-root
        tangent=d-n*d.dot(n)
        if tangent.length<1e-8:
            tangent=Vector((1.0 if root.x>=0 else -1.0,-.20,.08))
        tangent.normalize()
        ...
        cross.normalize()
        p0=root+n*.00045
        q1,n1=c41_project(root+tangent*.014+n*.0010,n)
        q2,n2=c41_project(root+tangent*.030+n*.0014,n1)
        q3,n3=c41_project(root+tangent*.047+n*.0017,n2)

        # Dense short scalp-hugging fibers provide coverage instead of vertical comb lines.
        front_band=(root.y>eye_center.y-.012 and root.z<hairline+.034)
        offsets=(0.0,) if ((C42_EYE_GEOMETRY_SKIN or C43_HYBRID_REALISM) and front_band) else (-.0012,0.0,.0012)
        for off in offsets:
            r0,_=c41_project(p0+cross*off,n)
            r1,_=c41_project(q1+cross*off,n1)
            r2,_=c41_project(q2+cross*off,n2)
            r3,_=c41_project(q3+cross*off,n3)
            undercoat.append([r0,r1,r2,r3])
        gather=q3.lerp(target,.56)+Vector((0,0,.0035))
        primary.append([p0,q1,q2,q3,gather,target])
    curve_object("DIGE_C41_HHIR_SCALP_UNDERCOAT",undercoat,.000034 if C42_EYE_GEOMETRY_SKIN else .000040,hair)
    curve_object("DIGE_C41_HHIR_SCALP_GATHER",primary,.000027 if C42_EYE_GEOMETRY_SKIN else .000030,hair)

    # Rounded, distributed bun loops. Targets are deliberately de-correlated so
    # the groom cannot collapse into the C39 box/fan singularity.
    bun=[]
    bun_count=420
    for i in range(bun_count):
        phi=math.tau*((i*.61803398875)%1.0)
        tilt=hrng.uniform(-.55,.55)
        b1=Vector((math.cos(phi),.48*math.sin(phi),.24*tilt))
        if b1.length<1e-8: b1=Vector((1,0,0))
        b1.normalize()
        b2=Vector((-.42*math.sin(phi),.34*math.cos(phi),.88))
        b2=b2-b1*b2.dot(b1)
        if b2.length<1e-8: b2=Vector((0,0,1))
        b2.normalize()
        center=bun_center+Vector((hrng.uniform(-.010,.010),hrng.uniform(-.007,.007),hrng.uniform(-.012,.012)))
        rr=hrng.uniform(.026,.044)
        squash=hrng.uniform(.54,.78)
        pts=[]
        for j in range(17):
            a=math.tau*j/16.0
            p=center+b1*(rr*math.cos(a))+b2*(rr*squash*math.sin(a))
            pts.append(tuple(p))
        bun.append(pts)
    curve_object("DIGE_C40_HHIR_ROUNDED_BUN",bun,.000038,hair)

    # C41 hairline feathers are sparse and sweep tangentially/backward; they do not
    # form the vertical forehead comb visible in C40.
    front=[q for q in scalp if q[1].y > eye_center.y-.010 and q[1].z < hairline+.026]
    front.sort(key=lambda q:(q[1].x,q[0]))
    fringe=[]
    if front:
        fcount=min(28 if C43_HYBRID_REALISM else 56,len(front))
        picks=[front[min(len(front)-1,int(i*len(front)/max(1,fcount)))] for i in range(fcount)]
        for _vi,root,n in picks:
            side=1.0 if root.x>=0 else -1.0
            temple=min(1.0,abs(root.x)/.09)
            length=hrng.uniform(.008,.019)*(1.0-.20*temple)
            p0=root+n*.00055
            lateral=Vector((side*.55,-.78,.22))
            lateral=lateral-n*lateral.dot(n)
            if lateral.length<1e-8:
                lateral=Vector((side,-.5,.2))
            lateral.normalize()
            fringe.append([
                p0,
                p0+lateral*(length*.34)+n*.0007,
                p0+lateral*(length*.70)+n*.0009,
                p0+lateral*length+n*.0006,
            ])
        curve_object("DIGE_C41_HHIR_HAIRLINE_FEATHERS",fringe,.000022,hair)

    fly=[]
    for i in range(32):
        phase=math.tau*((i*.754877666)%1.0)
        root=bun_center+Vector((.036*math.cos(phase),.022*math.sin(phase),.028*math.sin(phase*.7)))
        length=hrng.uniform(.012,.032)
        side=1.0 if math.cos(phase)>=0 else -1.0
        fly.append([
            root,
            root+Vector((side*length*.18,-length*.10,length*.30)),
            root+Vector((side*length*.36,-length*.18,length*.68)),
        ])
    curve_object("DIGE_C40_HHIR_FLYAWAYS",fly,.000020,hair)

    c39_updo_metrics={
        "enabled":True,
        "style":"C41_HHIR_NATIVE_EYE_SCALP_FLOW_UPDO_V1",
        ...
        "bun_radius_range_m":[.026,.044],
        "camera_fixed_85mm":True,
        "truth_boundary":"SOURCE_LEVEL_GNM_SCALP_ROOTS__PROCEDURAL_CURVES__NO_REFERENCE_PIXELS_USED"
    }
...
if C41_NATIVE_EYE_SCALP_FLOW:
    hair_style_label="C41_HHIR_NATIVE_EYE_SCALP_FLOW_UPDO_V1"
...
if C43_HYBRID_REALISM and RENDER_SET=="HERO_ONLY":
...
receipt={
 ...
 "appearance_candidate":(
   "C45_HHIR_BARYCENTRIC_UV_SKIN_V1" if C45_BARYCENTRIC_UV_SKIN else
   "C44_HHIR_UV_TRANSFER_SKIN_V1" if C44_UV_TRANSFER_SKIN else
   "C43_HHIR_HYBRID_REALISM_V1" if C43_HYBRID_REALISM else
   "C42_HHIR_EYE_GEOMETRY_FACE_SKIN_V1" if C42_EYE_GEOMETRY_SKIN else
   "C41_HHIR_NATIVE_EYE_SCALP_FLOW_V1" if C41_NATIVE_EYE_SCALP_FLOW else
 ...
 "appearance_selection":{
   ...
   "c41_native_eye_scalp_flow":C41_NATIVE_EYE_SCALP_FLOW,
   "c42_eye_geometry_skin":C42_EYE_GEOMETRY_SKIN,
   "c43_hybrid_realism":C43_HYBRID_REALISM,
   "c44_uv_transfer_skin":C44_UV_TRANSFER_SKIN,
   "c45_barycentric_uv_skin":C45_BARYCENTRIC_UV_SKIN,
   "c44_uv_transfer_contract":c44_uv_transfer_contract,
