...
if C39_HHIR_HYPERREAL:
    ...
    for vi,root,n in roots:
        ...
        p0=root+n*.0007
        p1=root.lerp(target,.28)+n*.008
        p2=root.lerp(target,.62)+n*.011+Vector((0,0,.004))
        p3=root.lerp(target,.86)+Vector((0,0,.006))
        primary.append([p0,p1,p2,p3,target])
    curve_object("DIGE_C40_HHIR_SCALP_GATHER",primary,.000034,hair)
    ...
    # Actual hairline-rooted micro fringe / baby hair.
    front=[q for q in scalp if q[1].y > eye_center.y-.010 and q[1].z < hairline+.030]
    front.sort(key=lambda q:(q[1].x,q[0]))
    fringe=[]
    if front:
        fcount=min(360,max(120,len(front)))
        ...
        for _vi,root,n in picks:
            ...
            length=hrng.uniform(.010,.035)*(1.0-.25*temple)
            p0=root+n*.0006
            fringe.append([
                p0,
                p0+Vector((side*.0015,-.003,-length*.22)),
                p0+Vector((side*.0030,-.006,-length*.55)),
                p0+Vector((side*.0045,-.009,-length)),
            ])
        curve_object("DIGE_C40_HHIR_HAIRLINE_MICRO",fringe,.000026,hair)
    ...
    c39_updo_metrics={
        "enabled":True,
        "style":"C40_HHIR_GNM_SCALP_BOUND_DISTRIBUTED_UPDO_V1",
