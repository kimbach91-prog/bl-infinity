...
def c28_gnm_skin_material():
    ...
    if C31_GNM_FACE_APPEARANCE:
        ...
        bump=nt.nodes.new("ShaderNodeBump")
        bump.inputs["Strength"].default_value=.54 if C43_TANGENTIAL_GROOM else (.48 if C42_GEOMETRY_EYE_HAIRLINE else (.40 if C41_HYPERREAL_NATIVE_EYE else (.24 if C36_CANONICAL_APPEARANCE else (.34 if C32_GNM_DERMAL_HAIR else .24))))
        bump.inputs["Distance"].default_value=.000080 if C43_TANGENTIAL_GROOM else (.000065 if C42_GEOMETRY_EYE_HAIRLINE else (.000058 if C41_HYPERREAL_NATIVE_EYE else (.000038 if C36_CANONICAL_APPEARANCE else (.000052 if C32_GNM_DERMAL_HAIR else .000070))))
...
if C28_GNM_HEAD:
    ...
    if C42_GEOMETRY_EYE_HAIRLINE:
        c28_skin_contract.update({
            ...
            "metadata_effective_for":("C43" if C43_TANGENTIAL_GROOM else "C42"),
        })
        if C43_TANGENTIAL_GROOM:
            c28_skin_contract.update({
                "model":"C43_GNM_VISIBLE_SCALE_PBR_SKIN",
                "subsurface_weight":0.006,
                "pore_scale":330.0,
                "micro_scale":1050.0,
                "bump_distance":0.000080,
            })
    if C29_GNM_PRESENTATION:
        if C42_GEOMETRY_EYE_HAIRLINE:
    ...
    target_hairline_z=brow_top+(.026 if C42_GEOMETRY_EYE_HAIRLINE else (.028 if C41_HYPERREAL_NATIVE_EYE else .043))
    front=[]
...
if C41_HYPERREAL_NATIVE_EYE:
    ...
    brow_top=max(p.z for p in (gnm_brow_left+gnm_brow_right))
    hairline=min(float(c28_gnm["hairline_target_z"]),brow_top+.029)
    ...
    for v in groom_surface.data.vertices:
        ...
        edge_z=hairline+.010*(temple**1.55)
    ...
    for vi,root,n,_edge in scalp:
        # C43 coverage fibers follow the scalp backward/upward, not directly to bun.
        tangent=Vector((0.0,-1.0,.18))
        tangent=tangent-n*tangent.dot(n)
        if tangent.length<1e-8:
            tangent=Vector((0,-1,.10))
        tangent.normalize()
        ...
        cross.normalize()
        p0=root+n*.00050
        L=hrng.uniform(.028,.045)
        undercoat.append([
            p0,
            p0+tangent*(L*.30)+n*.0015,
            p0+tangent*(L*.64)+n*.0022+cross*hrng.uniform(-.0008,.0008),
            p0+tangent*L+n*.0016,
        ])
    curve_object("DIGE_C43_HHIR_TANGENTIAL_UNDERCOAT",undercoat,.000024,hair)

    gather_pool=[q for q in scalp if (q[1].z >= q[3]+.020 or q[1].y < eye_center.y-.010)]
    if len(gather_pool)<350:
        raise RuntimeError(f"C43 gather pool too sparse: {len(gather_pool)}")
    primary_count=min(900,len(gather_pool))
    ...
    curve_object("DIGE_C43_HHIR_TANGENTIAL_GATHER",primary,.000028,hair)
    ...
    micro=[]
    if edge_band:
        fcount=min(90,max(45,len(edge_band)))
        ...
        curve_object("DIGE_C43_HHIR_HAIRLINE_EDGE",micro,.000016,hair)
    ...
    c41_groom_metrics={
        "enabled":True,
        "style":"C43_TANGENTIAL_SCALP_FLOW_GEOMETRY_EYE_V1",
        "hairline_z":hairline,
