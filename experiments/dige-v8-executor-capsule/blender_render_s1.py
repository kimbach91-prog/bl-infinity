...
def s1_gnm_skin_material():
    ...
    ramp.color_ramp.elements[0].color=(0.19,0.11,0.09,1)
    ramp.color_ramp.elements[1].position=.84
    ramp.color_ramp.elements[1].color=(0.385,0.25,0.205,1)
...
if C41_HYPERREAL_NATIVE_EYE:
    ...
    for vi,root,n,_edge in scalp:
        ...
        for lane in range(lane_count):
            ...
            L=hrng.uniform(.024,.042) if S1_ENDOGENOUS else (hrng.uniform(.032,.052) if C44_FOLLICLE_FLOW else hrng.uniform(.028,.045))
            tangent=(base_t+cross*hrng.uniform(-.18,.18)+n*hrng.uniform(-.01,.025)).normalized() if S1_ENDOGENOUS else (base_t+cross*hrng.uniform(-.10,.10)).normalized()
    ...
    curve_object("DIGE_S1_DENSE_TANGENTIAL_UNDERCOAT" if S1_ENDOGENOUS else ("DIGE_C44_HHIR_FOLLICLE_UNDERCOAT" if C44_FOLLICLE_FLOW else "DIGE_C43_HHIR_TANGENTIAL_UNDERCOAT"),undercoat,.000013 if S1_ENDOGENOUS else (.000019 if C44_FOLLICLE_FLOW else .000024),hair)

    gather_pool=[q for q in scalp if (q[1].z >= q[3]+(.040 if S1_ENDOGENOUS else (.030 if C44_FOLLICLE_FLOW else .020)) or q[1].y < eye_center.y-.020)]
    if len(gather_pool)<350:
        raise RuntimeError(f"C43 gather pool too sparse: {len(gather_pool)}")
    primary_count=min(600 if S1_ENDOGENOUS else (720 if C44_FOLLICLE_FLOW else 900),len(gather_pool))
    ...
    micro=[]
    if edge_band:
        fcount=min(80,max(40,len(edge_band))) if S1_ENDOGENOUS else (min(48,max(24,len(edge_band))) if C44_FOLLICLE_FLOW else min(90,max(45,len(edge_band))))
        ...
        curve_object("DIGE_S1_HAIRLINE_BABY" if S1_ENDOGENOUS else ("DIGE_C44_HHIR_HAIRLINE_EDGE" if C44_FOLLICLE_FLOW else "DIGE_C43_HHIR_HAIRLINE_EDGE"),micro,.000011 if S1_ENDOGENOUS else (.000013 if C44_FOLLICLE_FLOW else .000016),hair)
    ...
    if S1_ENDOGENOUS:
        ...
        if frame_roots:
            ...
            for _vi,root,n,_edge in frame_roots[::step][:40]:
                ...
                frame.append(pts)
            if frame:
                curve_object("DIGE_S1_FACE_FRAME_WISPS",frame,.000012,hair)
                s1_face_frame_curves=len(frame)

    c41_groom_metrics={
        "enabled":True,
        "style":("S1_LAYERED_SCALP_BUN_GROOM_V1" if S1_ENDOGENOUS else ("C44_FOLLICLE_FLOW_GEOMETRY_EYE_V1" if C44_FOLLICLE_FLOW else "C43_TANGENTIAL_SCALP_FLOW_GEOMETRY_EYE_V1")),
        "hairline_z":hairline,
...
all_views=[
 ("01_FRONT50",(0,5.1,1.03),(0,0,.92),50,7.1,(640 if S1_ENDOGENOUS else 512),(960 if S1_ENDOGENOUS else 768)),
...
receipt={
 ...
 "claim_ceiling":("S1_ENDOGENOUS_SOURCE_LEVEL_SYSTEM_EXECUTED; VISUAL_HYPERREAL_CERTIFICATION_REQUIRES_DIRECT_MULTIVIEW_AUDIT" if S1_ENDOGENOUS else "PRODUCTION_TOPOLOGY_PLUS_CYCLES_PATH_TRACED_CANDIDATE; HYPERREAL_CERTIFICATION_REQUIRES_VISUAL_AND_LHYPER_AUDIT; DEUS_IDENTITY_NOT_BOUND")
}
