...
if C28_GNM_HEAD:
    ...
    if C44_UV_TRANSFER_SKIN:
        ...
        src_sums=[Vector((0.0,0.0)) for _ in body.data.vertices]
        src_counts=[0 for _ in body.data.vertices]
        for poly in body.data.polygons:
            for li in poly.loop_indices:
                vi=body.data.loops[li].vertex_index
                src_sums[vi]+=Vector(src_uv[li].uv)
                src_counts[vi]+=1
        src_avg=[]
        kd=KDTree(len(body.data.vertices))
        for i,v in enumerate(body.data.vertices):
            uv=(src_sums[i]/src_counts[i]) if src_counts[i] else Vector((0.5,0.5))
            src_avg.append(uv)
            kd.insert(body.matrix_world @ v.co,i)
        kd.balance()
        uv_name="C44_TRANSFERRED_MAKEHUMAN_UV"
        ...
        for poly in skin_obj.data.polygons:
            for li in poly.loop_indices:
                ...
                _co,src_idx,dist=kd.find(wp)
                dst_layer.data[li].uv=src_avg[src_idx]
                distances.append(float(dist))
        if not distances:
            raise RuntimeError("C44 UV transfer produced no loop distances")
        ...
        c44mat=bpy.data.materials.new("DIGE_C44_GNM_TRANSFERRED_SKIN")
        ...
        c44_uv_transfer_contract={
            "enabled":True,
            "method":"NEAREST_SOURCE_VERTEX_AVERAGED_UV",
            ...
            "material":"DIGE_C44_GNM_TRANSFERRED_SKIN",
            "truth_boundary":"UV_TRANSFER_FROM_EXISTING_CC0_MAKEHUMAN_TEXTURE__NO_REFERENCE_IMAGE_PIXELS"
        }
        c28_skin_contract={
            "model":"C44_GNM_UV_TRANSFERRED_CC0_SKIN_V1",
