...
if C28_GNM_HEAD:
    ...
    target_hairline_z=brow_top+(.018 if C44_FOLLICLE_FLOW else (.026 if C42_GEOMETRY_EYE_HAIRLINE else (.028 if C41_HYPERREAL_NATIVE_EYE else .043)))
    front=[]
...
if C41_HYPERREAL_NATIVE_EYE:
    ...
    brow_top=max(p.z for p in (gnm_brow_left+gnm_brow_right))
    hairline=min(float(c28_gnm["hairline_target_z"]),brow_top+(.020 if C44_FOLLICLE_FLOW else .029))
    ...
    for v in groom_surface.data.vertices:
        ...
        edge_z=hairline+(.012 if C44_FOLLICLE_FLOW else .010)*(temple**1.55)
    ...
    for vi,root,n,_edge in scalp:
        ...
        if cross.length<1e-8:
            cross=Vector((1,0,0))
        cross.normalize()
        lane_count=2 if C44_FOLLICLE_FLOW else 1
        for lane in range(lane_count):
            lane_bias=(lane-(lane_count-1)*.5)*.00032
            p0=root+n*.00048+cross*lane_bias
            L=hrng.uniform(.032,.052) if C44_FOLLICLE_FLOW else hrng.uniform(.028,.045)
            tangent=(base_t+cross*hrng.uniform(-.10,.10)).normalized()
    ...
    curve_object("DIGE_C44_HHIR_FOLLICLE_UNDERCOAT" if C44_FOLLICLE_FLOW else "DIGE_C43_HHIR_TANGENTIAL_UNDERCOAT",undercoat,.000019 if C44_FOLLICLE_FLOW else .000024,hair)

    gather_pool=[q for q in scalp if (q[1].z >= q[3]+(.030 if C44_FOLLICLE_FLOW else .020) or q[1].y < eye_center.y-.014)]
    if len(gather_pool)<350:
        raise RuntimeError(f"C43 gather pool too sparse: {len(gather_pool)}")
    primary_count=min(720 if C44_FOLLICLE_FLOW else 900,len(gather_pool))
    ...
    curve_object("DIGE_C44_HHIR_DEEP_GATHER" if C44_FOLLICLE_FLOW else "DIGE_C43_HHIR_TANGENTIAL_GATHER",primary,.000025 if C44_FOLLICLE_FLOW else .000028,hair)
    ...
    micro=[]
    if edge_band:
        fcount=min(48,max(24,len(edge_band))) if C44_FOLLICLE_FLOW else min(90,max(45,len(edge_band)))
        ...
        curve_object("DIGE_C44_HHIR_HAIRLINE_EDGE" if C44_FOLLICLE_FLOW else "DIGE_C43_HHIR_HAIRLINE_EDGE",micro,.000013 if C44_FOLLICLE_FLOW else .000016,hair)
    ...
    c41_groom_metrics={
        "enabled":True,
        "style":("C44_FOLLICLE_FLOW_GEOMETRY_EYE_V1" if C44_FOLLICLE_FLOW else "C43_TANGENTIAL_SCALP_FLOW_GEOMETRY_EYE_V1"),
        "hairline_z":hairline,
        ...
        "undercoat_lane_count":(2 if C44_FOLLICLE_FLOW else 1),
...
if (C41_HYPERREAL_NATIVE_EYE or C42_GEOMETRY_EYE_HAIRLINE) and RENDER_SET=="HERO_ONLY":
