...
def c28_gnm_skin_material():
    ...
    if C31_GNM_FACE_APPEARANCE:
        ...
        set_input(bs,"IOR",1.42)
        set_input(bs,"Specular IOR Level",.26 if C41_HYPERREAL_NATIVE_EYE else (.34 if C36_CANONICAL_APPEARANCE else .28))
        set_input(bs,"Subsurface Weight",.012 if C41_HYPERREAL_NATIVE_EYE else (.030 if C36_CANONICAL_APPEARANCE else (.045 if C32_GNM_DERMAL_HAIR else .070)))
        set_input(bs,"Subsurface Scale",.00048 if C41_HYPERREAL_NATIVE_EYE else (.00090 if C36_CANONICAL_APPEARANCE else (.00064 if C32_GNM_DERMAL_HAIR else .00082)))
        ...
        base_noise.inputs["Scale"].default_value=6.2 if C41_HYPERREAL_NATIVE_EYE else (9.0 if C36_CANONICAL_APPEARANCE else (18.0 if C32_GNM_DERMAL_HAIR else 10.0))
        ...
        rough_noise.inputs["Scale"].default_value=42.0 if C41_HYPERREAL_NATIVE_EYE else (68.0 if C36_CANONICAL_APPEARANCE else (145.0 if C32_GNM_DERMAL_HAIR else 52.0))
        ...
        rough_map.inputs["From Min"].default_value=0.0
        rough_map.inputs["From Max"].default_value=1.0
        rough_map.inputs["To Min"].default_value=.47 if C41_HYPERREAL_NATIVE_EYE else (.38 if C36_CANONICAL_APPEARANCE else (.46 if C32_GNM_DERMAL_HAIR else .44))
        rough_map.inputs["To Max"].default_value=.69 if C41_HYPERREAL_NATIVE_EYE else (.57 if C36_CANONICAL_APPEARANCE else (.64 if C32_GNM_DERMAL_HAIR else .60))
        ...
        pore=nt.nodes.new("ShaderNodeTexNoise")
        pore.inputs["Scale"].default_value=520.0 if C41_HYPERREAL_NATIVE_EYE else (760.0 if C36_CANONICAL_APPEARANCE else (2400.0 if C32_GNM_DERMAL_HAIR else 1900.0))
        ...
        micro=nt.nodes.new("ShaderNodeTexNoise")
        micro.inputs["Scale"].default_value=1650.0 if C41_HYPERREAL_NATIVE_EYE else (2400.0 if C36_CANONICAL_APPEARANCE else (9000.0 if C32_GNM_DERMAL_HAIR else 6200.0))
        ...
        bump=nt.nodes.new("ShaderNodeBump")
        bump.inputs["Strength"].default_value=.40 if C41_HYPERREAL_NATIVE_EYE else (.24 if C36_CANONICAL_APPEARANCE else (.34 if C32_GNM_DERMAL_HAIR else .24))
        bump.inputs["Distance"].default_value=.000058 if C41_HYPERREAL_NATIVE_EYE else (.000038 if C36_CANONICAL_APPEARANCE else (.000052 if C32_GNM_DERMAL_HAIR else .000070))
...
if C28_GNM_HEAD:
    ...
    if C29_GNM_PRESENTATION and not C41_HYPERREAL_NATIVE_EYE:
        for comp,pts in (("left_eye",transformed_landmarks[36:42]),("right_eye",transformed_landmarks[42:48])):
            eye_mesh=c28_gnm_objects.get(comp)
            if eye_mesh is None:
    ...
    target_hairline_z=brow_top+(.028 if C41_HYPERREAL_NATIVE_EYE else .043)
    front=[]
...
if C41_HYPERREAL_NATIVE_EYE:
    ...
    for v in groom_surface.data.vertices:
        ...
        if p.z < hairline-.006 or p.z > float(bbox_max.z)+.004:
            continue
        if abs(p.x) > .134 or p.y > eye_center.y+.052:
            continue
        if p.z < hairline+.004 and p.y > eye_center.y+.040:
            continue
        if n.z < -.34:
            continue
        scalp.append((v.index,p,n))
    if len(scalp) < 700:
        raise RuntimeError(f"C41 scalp coverage too sparse: {len(scalp)}")
    scalp.sort(key=lambda q:q[0])

    undercoat=[]
    for vi,root,n in scalp:
        to=bun_center-root
    ...
    front=[q for q in scalp if q[1].y > eye_center.y-.016 and q[1].z < hairline+.020]
    front.sort(key=lambda q:(q[1].x,q[0]))
    micro=[]
    if front:
        fcount=min(240,max(140,len(front)))
        picks=[front[min(len(front)-1,int(i*len(front)/fcount))] for i in range(fcount)]
        for _vi,root,n in picks:
            side=1.0 if root.x>=0 else -1.0
            p0=root+n*.00055
            L=hrng.uniform(.010,.024)
            micro.append([
                p0,
                p0+Vector((side*.001,-.003,L*.35)),
                p0+Vector((side*.002,-.007,L*.70)),
                p0+Vector((side*.003,-.012,L)),
            ])
        curve_object("DIGE_C41_HHIR_HAIRLINE_MICRO",micro,.000022,hair)
    ...
    c41_groom_metrics={
        "enabled":True,
        "style":"C41_GNM_SCALP_UNDERCOAT_DISTRIBUTED_UPDO_V1",
        "hairline_z":hairline,
...
if C41_HYPERREAL_NATIVE_EYE and RENDER_SET=="HERO_ONLY":
