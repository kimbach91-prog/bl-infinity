...
def c28_gnm_skin_material():
    ...
    if C31_GNM_FACE_APPEARANCE:
        ...
        return m,{
            "model":"C42_GNM_VISIBLE_SCALE_PBR_SKIN_V1" if C42_EYE_GEOMETRY_SKIN else ("C36_CANONICAL_PBR_SKIN_V1" if C36_CANONICAL_APPEARANCE else ("C32_GNM_DERMAL_MULTISCALE_SKIN" if C32_GNM_DERMAL_HAIR else "C31_GNM_REGIONAL_RANDOM_WALK_SKIN")),
...
if C28_GNM_HEAD:
    ...
    if C42_EYE_GEOMETRY_SKIN:
        ...
        for comp,pts in (("left_eye",transformed_landmarks[36:42]),("right_eye",transformed_landmarks[42:48])):
            ...
            iris_r=max(.00345,min(.00435,eye_w*.205))
            pupil_r=iris_r*.365
            iy=front_y+.000035
            cylinder(f"DIGE_C42_{comp.upper()}_LIMBAL",(ec.x,iy,ec.z),iris_r*1.055,.000026,iris_ring)
            cylinder(f"DIGE_C42_{comp.upper()}_IRIS",(ec.x,iy+.000018,ec.z),iris_r,.000024,iris)
            cylinder(f"DIGE_C42_{comp.upper()}_PUPIL",(ec.x,iy+.000036,ec.z),pupil_r,.000022,black)
            c42_iris_count+=1
...
if C39_HHIR_HYPERREAL:
    ...
    for vi,root,n in roots:
        ...
        front_band=(root.y>eye_center.y-.006 and root.z<hairline+.026)
        offsets=(0.0,) if (C42_EYE_GEOMETRY_SKIN and front_band) else (-.0012,0.0,.0012)
        for off in offsets:
