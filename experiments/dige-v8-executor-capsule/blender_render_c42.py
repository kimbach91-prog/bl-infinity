...
def c28_gnm_skin_material():
    ...
    if C31_GNM_FACE_APPEARANCE:
        ...
        set_input(bs,"Subsurface Weight",.008 if C42_GEOMETRY_EYE_HAIRLINE else (.012 if C41_HYPERREAL_NATIVE_EYE else (.030 if C36_CANONICAL_APPEARANCE else (.045 if C32_GNM_DERMAL_HAIR else .070))))
        ...
        pore=nt.nodes.new("ShaderNodeTexNoise")
        pore.inputs["Scale"].default_value=420.0 if C42_GEOMETRY_EYE_HAIRLINE else (520.0 if C41_HYPERREAL_NATIVE_EYE else (760.0 if C36_CANONICAL_APPEARANCE else (2400.0 if C32_GNM_DERMAL_HAIR else 1900.0)))
        ...
        micro=nt.nodes.new("ShaderNodeTexNoise")
        micro.inputs["Scale"].default_value=1350.0 if C42_GEOMETRY_EYE_HAIRLINE else (1650.0 if C41_HYPERREAL_NATIVE_EYE else (2400.0 if C36_CANONICAL_APPEARANCE else (9000.0 if C32_GNM_DERMAL_HAIR else 6200.0)))
        ...
        bump=nt.nodes.new("ShaderNodeBump")
        bump.inputs["Strength"].default_value=.48 if C42_GEOMETRY_EYE_HAIRLINE else (.40 if C41_HYPERREAL_NATIVE_EYE else (.24 if C36_CANONICAL_APPEARANCE else (.34 if C32_GNM_DERMAL_HAIR else .24)))
        bump.inputs["Distance"].default_value=.000065 if C42_GEOMETRY_EYE_HAIRLINE else (.000058 if C41_HYPERREAL_NATIVE_EYE else (.000038 if C36_CANONICAL_APPEARANCE else (.000052 if C32_GNM_DERMAL_HAIR else .000070)))
...
def c42_geometry_eye_material(name):
    ...
    elems[1].position=.095; elems[1].color=(0.002,0.0015,0.001,1)
    for pos,col in [
    ...
    return m,{
        ...
        "pupil_radius_norm":.095,
        "iris_outer_radius_norm":.270,
        "sclera_start_norm":.282,
        "uv_dependency":False,
    }
...
if C41_HYPERREAL_NATIVE_EYE:
    ...
    for vi,root,n,_edge in scalp:
        ...
        cross.normalize()
        for lane in (-1.0,1.0):
            p0=root+n*.00055+cross*(lane*.00028)
            L=hrng.uniform(.032,.052)
            undercoat.append([
                p0,
                p0+tangent*(L*.30)+n*.0020,
                p0+tangent*(L*.62)+n*.0030,
                p0+tangent*L+n*.0020,
            ])
    curve_object("DIGE_C41_HHIR_SCALP_UNDERCOAT",undercoat,.000030,hair)

    primary_count=min(1200,len(scalp))
    roots=[scalp[min(len(scalp)-1,int(i*len(scalp)/primary_count))] for i in range(primary_count)]
    primary=[]
    for vi,root,n,_edge in roots:
        ...
        p0=root+n*.00065
        p1=root.lerp(target,.24)+n*.007
        p2=root.lerp(target,.52)+n*.010+Vector((0,0,.003))
        p3=root.lerp(target,.78)+Vector((0,0,.007))
        primary.append([p0,p1,p2,p3,target])
    curve_object("DIGE_C41_HHIR_SCALP_GATHER",primary,.000034,hair)
    ...
    for i in range(520):
        ...
        bun.append(pts)
    curve_object("DIGE_C41_HHIR_ROUNDED_BUN",bun,.000036,hair)

    micro=[]
    if edge_band:
        fcount=min(180,max(90,len(edge_band)))
        ...
        curve_object("DIGE_C42_HHIR_HAIRLINE_EDGE",micro,.000020,hair)
    ...
    c41_groom_metrics={
        "enabled":True,
        "style":"C42_EDGE_GATED_SCALP_UNDERCOAT_GEOMETRY_EYE_V1",
        "hairline_z":hairline,
