...
def c25_apply_mpfb_enhanced_skin(material):
    ...
    if C40_HYPERREAL_REPAIR:
        settings.update({
            "Clearcoat":0.006,
            "Clearcoat Roughness":0.52,
            "Pore detail":5.0,
            "Pore distortion":1.35,
            "Pore scale":3900.0,
            "Pore strength":0.48,
            "Roughness":0.59,
            "colorMixInStrength":0.006,
            "SSS strength":0.035,
            "SSS radius scale":0.042,
        })
...
def c25_apply_mpfb_procedural_eyes(material):
    ...
    if C41_HYPERREAL_NATIVE_EYE:
        settings.update({
            "EyeWhiteColor":(0.72,0.67,0.62,1.0),
            "InnerLayerRoughness":0.08,
            "IrisBumpStrength":0.34,
            "IrisMajorColor":(0.11,0.035,0.012,1.0),
            "IrisMinorColor":(0.035,0.010,0.004,1.0),
            "IrisToEyeWhiteRelation":0.41,
            "OuterLayerRoughness":0.018,
            "PupilSize":0.27,
        })
    if C40_HYPERREAL_REPAIR:
        settings.update({
            "EyeWhiteColor":(0.82,0.78,0.74,1.0),
            "IrisBumpStrength":0.42,
            "IrisToEyeWhiteRelation":0.48,
            "PupilSize":0.34,
            "OuterLayerRoughness":0.018,
        })
    c25_set_group_values(group,settings)
...
C40_HYPERREAL_REPAIR=os.environ.get("DIGE_C40_HYPERREAL_REPAIR","0").strip()=="1"
C41_HYPERREAL_NATIVE_EYE=os.environ.get("DIGE_C41_HYPERREAL_NATIVE_EYE","0").strip()=="1"
C42_GEOMETRY_EYE_HAIRLINE=os.environ.get("DIGE_C42_GEOMETRY_EYE_HAIRLINE","0").strip()=="1"
C43_TANGENTIAL_GROOM=os.environ.get("DIGE_C43_TANGENTIAL_GROOM","0").strip()=="1"
...
def c28_gnm_skin_material():
    ...
    if C31_GNM_FACE_APPEARANCE:
        ...
        set_input(bs,"IOR",1.42)
        set_input(bs,"Specular IOR Level",.23 if C42_GEOMETRY_EYE_HAIRLINE else (.26 if C41_HYPERREAL_NATIVE_EYE else (.34 if C36_CANONICAL_APPEARANCE else .28)))
        set_input(bs,"Subsurface Weight",.006 if C43_TANGENTIAL_GROOM else (.008 if C42_GEOMETRY_EYE_HAIRLINE else (.012 if C41_HYPERREAL_NATIVE_EYE else (.030 if C36_CANONICAL_APPEARANCE else (.045 if C32_GNM_DERMAL_HAIR else .070)))))
        set_input(bs,"Subsurface Scale",.00035 if C42_GEOMETRY_EYE_HAIRLINE else (.00048 if C41_HYPERREAL_NATIVE_EYE else (.00090 if C36_CANONICAL_APPEARANCE else (.00064 if C32_GNM_DERMAL_HAIR else .00082))))
        ...
        set_input(bs,"Coat Weight",.0 if C41_HYPERREAL_NATIVE_EYE else (.0 if C36_CANONICAL_APPEARANCE else (.004 if C32_GNM_DERMAL_HAIR else .010)))
        set_input(bs,"Coat Roughness",.62 if C41_HYPERREAL_NATIVE_EYE else (.55 if C36_CANONICAL_APPEARANCE else (.50 if C32_GNM_DERMAL_HAIR else .44)))
        if hasattr(bs,"subsurface_method"):
            bs.subsurface_method='RANDOM_WALK_SKIN'

        tex=nt.nodes.new("ShaderNodeTexCoord")
        base_noise=nt.nodes.new("ShaderNodeTexNoise")
        base_noise.inputs["Scale"].default_value=4.8 if C42_GEOMETRY_EYE_HAIRLINE else (6.2 if C41_HYPERREAL_NATIVE_EYE else (9.0 if C36_CANONICAL_APPEARANCE else (18.0 if C32_GNM_DERMAL_HAIR else 10.0)))
        ...
        base_ramp.color_ramp.elements[0].color=((0.165,0.085,0.065,1) if C41_HYPERREAL_NATIVE_EYE else ((0.220,0.125,0.105,1) if C36_CANONICAL_APPEARANCE else ((0.205,0.073,0.048,1) if C32_GNM_DERMAL_HAIR else (0.245,0.082,0.042,1))))
        base_ramp.color_ramp.elements[1].position=.82
        base_ramp.color_ramp.elements[1].color=((0.325,0.195,0.150,1) if C41_HYPERREAL_NATIVE_EYE else ((0.365,0.225,0.190,1) if C36_CANONICAL_APPEARANCE else ((0.395,0.176,0.118,1) if C32_GNM_DERMAL_HAIR else (0.445,0.190,0.102,1))))
        ...
        rough_noise.inputs["Scale"].default_value=34.0 if C42_GEOMETRY_EYE_HAIRLINE else (42.0 if C41_HYPERREAL_NATIVE_EYE else (68.0 if C36_CANONICAL_APPEARANCE else (145.0 if C32_GNM_DERMAL_HAIR else 52.0)))
        ...
        rough_map.inputs["From Min"].default_value=0.0
        rough_map.inputs["From Max"].default_value=1.0
        rough_map.inputs["To Min"].default_value=.50 if C42_GEOMETRY_EYE_HAIRLINE else (.47 if C41_HYPERREAL_NATIVE_EYE else (.38 if C36_CANONICAL_APPEARANCE else (.46 if C32_GNM_DERMAL_HAIR else .44)))
        rough_map.inputs["To Max"].default_value=.72 if C42_GEOMETRY_EYE_HAIRLINE else (.69 if C41_HYPERREAL_NATIVE_EYE else (.57 if C36_CANONICAL_APPEARANCE else (.64 if C32_GNM_DERMAL_HAIR else .60)))
        nt.links.new(rough_noise.outputs["Fac"],rough_map.inputs["Value"])
        tzone_scale=nt.nodes.new("ShaderNodeMath"); tzone_scale.operation='MULTIPLY'; tzone_scale.inputs[1].default_value=.024 if C41_HYPERREAL_NATIVE_EYE else (.040 if C36_CANONICAL_APPEARANCE else .055)
        lip_rough_scale=nt.nodes.new("ShaderNodeMath"); lip_rough_scale.operation='MULTIPLY'; lip_rough_scale.inputs[1].default_value=.052 if C41_HYPERREAL_NATIVE_EYE else (.085 if C36_CANONICAL_APPEARANCE else .070)
        ...
        pore=nt.nodes.new("ShaderNodeTexNoise")
        pore.inputs["Scale"].default_value=330.0 if C43_TANGENTIAL_GROOM else (420.0 if C42_GEOMETRY_EYE_HAIRLINE else (520.0 if C41_HYPERREAL_NATIVE_EYE else (760.0 if C36_CANONICAL_APPEARANCE else (2400.0 if C32_GNM_DERMAL_HAIR else 1900.0))))
        ...
        micro=nt.nodes.new("ShaderNodeTexNoise")
        micro.inputs["Scale"].default_value=1050.0 if C43_TANGENTIAL_GROOM else (1350.0 if C42_GEOMETRY_EYE_HAIRLINE else (1650.0 if C41_HYPERREAL_NATIVE_EYE else (2400.0 if C36_CANONICAL_APPEARANCE else (9000.0 if C32_GNM_DERMAL_HAIR else 6200.0))))
        ...
        final_height=add.outputs[0]
        if C44_FOLLICLE_FLOW:
            meso=nt.nodes.new("ShaderNodeTexNoise")
            meso.inputs["Scale"].default_value=92.0
            ...
            meso_scale=nt.nodes.new("ShaderNodeMath"); meso_scale.operation='MULTIPLY'; meso_scale.inputs[1].default_value=.22
            nt.links.new(meso.outputs["Fac"],meso_scale.inputs[0])
            hsum=nt.nodes.new("ShaderNodeMath"); hsum.operation='ADD'
            nt.links.new(add.outputs[0],hsum.inputs[0]); nt.links.new(meso_scale.outputs[0],hsum.inputs[1])
            final_height=hsum.outputs[0]
        bump=nt.nodes.new("ShaderNodeBump")
        bump.inputs["Strength"].default_value=.60 if C44_FOLLICLE_FLOW else (.54 if C43_TANGENTIAL_GROOM else (.48 if C42_GEOMETRY_EYE_HAIRLINE else (.40 if C41_HYPERREAL_NATIVE_EYE else (.24 if C36_CANONICAL_APPEARANCE else (.34 if C32_GNM_DERMAL_HAIR else .24)))))
        bump.inputs["Distance"].default_value=.000095 if C44_FOLLICLE_FLOW else (.000080 if C43_TANGENTIAL_GROOM else (.000065 if C42_GEOMETRY_EYE_HAIRLINE else (.000058 if C41_HYPERREAL_NATIVE_EYE else (.000038 if C36_CANONICAL_APPEARANCE else (.000052 if C32_GNM_DERMAL_HAIR else .000070)))))
        nt.links.new(final_height,bump.inputs["Height"])
        nt.links.new(bump.outputs["Normal"],bs.inputs["Normal"])
...
def s1_gnm_skin_material():
    """S1 source-level GNM skin: regional color + roughness + meso/pore/micro relief."""
    m=bpy.data.materials.new("DIGE_S1_GNM_SKIN")
    m.use_nodes=True
    nt=m.node_tree
    nt.nodes.clear()
    out=nt.nodes.new("ShaderNodeOutputMaterial")
    ...
    nt.links.new(sep.outputs["Green"],blushmix.inputs[0])
    nt.links.new(lipmix.outputs["Color"],blushmix.inputs[1])
    nt.links.new(blushc.outputs[0],blushmix.inputs[2])
    ...
    nt.links.new(sep.outputs["Blue"],tzone.inputs[0])
    nt.links.new(sep.outputs["Red"],liprough.inputs[0])
    rs1=nt.nodes.new("ShaderNodeMath"); rs1.operation='SUBTRACT'
    rs2=nt.nodes.new("ShaderNodeMath"); rs2.operation='SUBTRACT'
    nt.links.new(rmap.outputs["Result"],rs1.inputs[0]); nt.links.new(tzone.outputs[0],rs1.inputs[1])
    nt.links.new(rs1.outputs[0],rs2.inputs[0]); nt.links.new(liprough.outputs[0],rs2.inputs[1])
    ...
    for node in (meso,pore,micro):
        nt.links.new(tex.outputs["Object"],node.inputs["Vector"])
    ms=nt.nodes.new("ShaderNodeMath"); ms.operation='MULTIPLY'; ms.inputs[1].default_value=.12
    ps=nt.nodes.new("ShaderNodeMath"); ps.operation='MULTIPLY'; ps.inputs[1].default_value=.56
    xs=nt.nodes.new("ShaderNodeMath"); xs.operation='MULTIPLY'; xs.inputs[1].default_value=.26
    nt.links.new(meso.outputs["Fac"],ms.inputs[0]); nt.links.new(pore.outputs["Fac"],ps.inputs[0]); nt.links.new(micro.outputs["Fac"],xs.inputs[0])
...
def s1_geometry_eye_material(name):
    """S1 native GNM eye, generated-coordinate iris/pupil, no UV and no overlay discs."""
    ...
    dx=nt.nodes.new("ShaderNodeMath"); dx.operation='SUBTRACT'; dx.inputs[1].default_value=.5
    dz=nt.nodes.new("ShaderNodeMath"); dz.operation='SUBTRACT'; dz.inputs[1].default_value=.5
    nt.links.new(sep.outputs["X"],dx.inputs[0]); nt.links.new(sep.outputs["Z"],dz.inputs[0])
    dx2=nt.nodes.new("ShaderNodeMath"); dx2.operation='MULTIPLY'
    dz2=nt.nodes.new("ShaderNodeMath"); dz2.operation='MULTIPLY'
    nt.links.new(dx.outputs[0],dx2.inputs[0]); nt.links.new(dx.outputs[0],dx2.inputs[1])
    nt.links.new(dz.outputs[0],dz2.inputs[0]); nt.links.new(dz.outputs[0],dz2.inputs[1])
    sm=nt.nodes.new("ShaderNodeMath"); sm.operation='ADD'
    nt.links.new(dx2.outputs[0],sm.inputs[0]); nt.links.new(dz2.outputs[0],sm.inputs[1])
    rad=nt.nodes.new("ShaderNodeMath"); rad.operation='POWER'; rad.inputs[1].default_value=.5
    nt.links.new(sm.outputs[0],rad.inputs[0])
...
def c42_geometry_eye_material(name):
    """Topology/UV-independent eye material using normalized Generated coordinates.

    The visible iris/pupil is derived from radial distance in the native eyeball's
    generated X/Z plane. This keeps the iris embedded in the GNM eye mesh and does
    not create floating geometric discs.
    """
    m=bpy.data.materials.new(name)
    m.use_nodes=True
    nt=m.node_tree
    ...
    out=nt.nodes.new("ShaderNodeOutputMaterial")
    bs=nt.nodes.new("ShaderNodeBsdfPrincipled")
    set_input(bs,"Roughness",.30)
    set_input(bs,"IOR",1.376)
    set_input(bs,"Specular IOR Level",.34)
    set_input(bs,"Coat Weight",.20)
    set_input(bs,"Coat Roughness",.045)
    set_input(bs,"Subsurface Weight",.006)
    ...
    ramp.color_ramp.interpolation='CONSTANT'
    elems=ramp.color_ramp.elements
    elems[0].position=0.0; elems[0].color=(0.002,0.0015,0.001,1)
    elems[1].position=(.080 if C43_TANGENTIAL_GROOM else .095); elems[1].color=(0.002,0.0015,0.001,1)
    _eye_stops=([
        (.090,(0.018,0.006,0.0025,1)),
        (.155,(0.095,0.030,0.010,1)),
        (.215,(0.040,0.010,0.0035,1)),
        (.235,(0.010,0.003,0.0015,1)),
        (.250,(0.56,0.50,0.46,1)),
        (.500,(0.62,0.57,0.53,1)),
    ] if C43_TANGENTIAL_GROOM else [
        (.105,(0.018,0.006,0.0025,1)),
        (.185,(0.095,0.030,0.010,1)),
        (.245,(0.040,0.010,0.0035,1)),
        (.270,(0.010,0.003,0.0015,1)),
        (.282,(0.56,0.50,0.46,1)),
        (.500,(0.62,0.57,0.53,1)),
    ])
    for pos,col in _eye_stops:
        e=elems.new(pos); e.color=col
    nt.links.new(rad.outputs[0],ramp.inputs["Fac"])
    nt.links.new(ramp.outputs["Color"],bs.inputs["Base Color"])

    # Subtle radial iris relief, still topology independent.
    wave=nt.nodes.new("ShaderNodeTexWave")
    ...
    return m,{
        "enabled":True,
        "material_model":"C42_NATIVE_GNM_GENERATED_COORD_EYE",
        "integration":"GNM_NATIVE_EYEBALL_MESH_GENERATED_COORD_RADIAL_IRIS_NO_UV_NO_OVERLAY",
        "pupil_radius_norm":(.080 if C43_TANGENTIAL_GROOM else .095),
        "iris_outer_radius_norm":(.235 if C43_TANGENTIAL_GROOM else .270),
        "sclera_start_norm":(.250 if C43_TANGENTIAL_GROOM else .282),
        "uv_dependency":False,
    }
...
def s1_cloth_material(name,base):
    ...
    out=nt.nodes.new("ShaderNodeOutputMaterial")
    bs=nt.nodes.new("ShaderNodeBsdfPrincipled")
    set_input(bs,"Base Color",(*base,1))
    set_input(bs,"Roughness",.72)
    set_input(bs,"Specular IOR Level",.22)
    set_input(bs,"Sheen Weight",.16)
    tex=nt.nodes.new("ShaderNodeTexCoord")
    n1=nt.nodes.new("ShaderNodeTexNoise"); n1.inputs["Scale"].default_value=260.0; n1.inputs["Detail"].default_value=2.0; n1.inputs["Roughness"].default_value=.60
    n2=nt.nodes.new("ShaderNodeTexWave"); n2.wave_type='BANDS'; n2.bands_direction='X'; n2.inputs["Scale"].default_value=200.0; n2.inputs["Distortion"].default_value=1.8
    nt.links.new(tex.outputs["Object"],n1.inputs["Vector"]); nt.links.new(tex.outputs["Object"],n2.inputs["Vector"])
    mul=nt.nodes.new("ShaderNodeMath"); mul.operation='MULTIPLY'
    nt.links.new(n1.outputs["Fac"],mul.inputs[0]); nt.links.new(n2.outputs["Color"],mul.inputs[1])
    bump=nt.nodes.new("ShaderNodeBump"); bump.inputs["Strength"].default_value=.14; bump.inputs["Distance"].default_value=.00016
    nt.links.new(mul.outputs[0],bump.inputs["Height"]); nt.links.new(bump.outputs["Normal"],bs.inputs["Normal"])
    nt.links.new(bs.outputs[0],out.inputs["Surface"])
    return m
...
def s2_anatomy_rest_shape(obj):
    metrics={"enabled":False,"vertices_touched":0,"max_offset_m":0.0}
    if not S2_ANATOMY_DYNAMICS:
        return metrics
    touched=0; maxoff=0.0
    for v in obj.data.vertices:
        p=v.co
        x=float(p.x); y=float(p.y); z=float(p.z); ax=abs(x)
        dx=dy=dz=0.0
        # Front anatomy fields.
        deltoid=math.exp(-0.5*((z-1.34)/.085)**2)*math.exp(-0.5*((ax-.205)/.055)**2)
        chest=math.exp(-0.5*((z-1.245)/.105)**2)*math.exp(-0.5*((ax-.105)/.085)**2)*max(0.0,min(1.0,(y+.01)/.16))
        core=math.exp(-0.5*((z-1.075)/.16)**2)*math.exp(-0.5*(ax/.115)**2)*max(0.0,min(1.0,(y+.005)/.13))
        hip=math.exp(-0.5*((z-.90)/.14)**2)*math.exp(-0.5*((ax-.185)/.10)**2)
        thigh=math.exp(-0.5*((z-.64)/.16)**2)*math.exp(-0.5*((ax-.145)/.085)**2)
        calf=math.exp(-0.5*((z-.31)/.115)**2)*math.exp(-0.5*((ax-.105)/.055)**2)
        # Posterior anatomy fields.
        scap=math.exp(-0.5*((z-1.30)/.11)**2)*math.exp(-0.5*((ax-.105)/.070)**2)*max(0.0,min(1.0,(-y+.01)/.16))
        lat=math.exp(-0.5*((z-1.16)/.14)**2)*math.exp(-0.5*((ax-.155)/.075)**2)*max(0.0,min(1.0,(-y+.01)/.16))
        glute=math.exp(-0.5*((z-.88)/.12)**2)*math.exp(-0.5*((ax-.135)/.105)**2)*max(0.0,min(1.0,(-y+.005)/.18))
        ham=math.exp(-0.5*((z-.61)/.18)**2)*math.exp(-0.5*((ax-.145)/.090)**2)*max(0.0,min(1.0,(-y+.005)/.15))
        side=1.0 if x>=0 else -1.0
        dx += side*(.0042*deltoid+.0030*hip+.0022*thigh+.0018*calf)
        dy += .0045*chest + .0014*core - .0032*scap - .0026*lat - .0050*glute - .0024*ham
        # Gentle tissue gravity / compression, bounded to millimeters.
        dz += -.0012*glute-.0008*thigh-.0005*chest
        off=Vector((dx,dy,dz))
        if off.length>1e-7:
            p += off
            touched+=1
            maxoff=max(maxoff,float(off.length))
    obj.data.update()
    metrics.update({
        "enabled":True,
        "vertices_touched":touched,
        "max_offset_m":maxoff,
        "fields":["deltoid","chest","core","hip","thigh","calf","scapula","lat","glute","hamstring"],
        "soft_tissue_dynamic":"NOT_EXECUTED",
        "rest_volume_target":[0.95,1.05]
    })
    return metrics
...
if C28_GNM_HEAD:
    ...
    if C42_GEOMETRY_EYE_HAIRLINE and not S1_ENDOGENOUS:
        c28_skin_contract.update({
            ...
            "subsurface_weight":0.008,
            "subsurface_scale":0.00035,
            "pore_scale":420.0,
            "micro_scale":1350.0,
            "bump_distance":0.000065,
            "metadata_effective_for":("C44" if C44_FOLLICLE_FLOW else ("C43" if C43_TANGENTIAL_GROOM else "C42")),
        })
        if C43_TANGENTIAL_GROOM:
            c28_skin_contract.update({
                "model":"C44_GNM_MESO_FOLLICLE_PBR_SKIN" if C44_FOLLICLE_FLOW else "C43_GNM_VISIBLE_SCALE_PBR_SKIN",
                "subsurface_weight":0.006,
                "pore_scale":330.0,
                "micro_scale":1050.0,
                "bump_distance":0.000095 if C44_FOLLICLE_FLOW else 0.000080,
                "meso_scale":92.0 if C44_FOLLICLE_FLOW else None,
            })
    if C29_GNM_PRESENTATION:
        if S1_ENDOGENOUS:
            gnm_eye_mat,c28_eye_contract=s1_geometry_eye_material("DIGE_S1_GNM_NATIVE_EYE")
        elif C42_GEOMETRY_EYE_HAIRLINE:
            gnm_eye_mat,c28_eye_contract=c42_geometry_eye_material("DIGE_C42_GNM_NATIVE_GEOMETRY_EYE")
        elif C41_HYPERREAL_NATIVE_EYE:
            gnm_eye_mat=bpy.data.materials.new("DIGE_C41_GNM_NATIVE_PROCEDURAL_EYE")
            gnm_eye_mat.use_nodes=True
            c28_eye_contract=c25_apply_mpfb_procedural_eyes(gnm_eye_mat)
            c28_eye_contract.update({
                "material_model":"C41_GNM_NATIVE_PROCEDURAL_EYE",
                "integration":"GNM_NATIVE_EYEBALL_MESH_NO_GEOMETRIC_IRIS_OVERLAY",
            })
        else:
            gnm_eye_mat=sclera
            c28_eye_contract={
    ...
    if C36_CANONICAL_APPEARANCE:
        ...
        if S2_ANATOMY_DYNAMICS:
            for v in skin_obj.data.vertices:
                ...
                if p.z < mouth_center.z+.006:
                    t=max(0.0,min(1.0,(mouth_center.z+.006-p.z)/max(.04,mouth_center.z+.006-chin.z)))
                    p.x=gnm_eye_mid.x+(p.x-gnm_eye_mid.x)*(1.0-.10*(t**1.2))
                # Neutral lip seal and less droop.
                mdx=abs(p.x-mouth_center.x); mdz=abs(p.z-mouth_center.z)
                ...
                p.x=gnm_eye_mid.x+(p.x-gnm_eye_mid.x)*(1.0-.055*nw)
                p.y+=.0008*nw
                # Ears less protrusive.
                earw=max(0.0,min(1.0,(abs(p.x-gnm_eye_mid.x)-.072)/.040))*math.exp(-0.5*((p.z-eye_z)/.065)**2)
                p.x=gnm_eye_mid.x+(p.x-gnm_eye_mid.x)*(1.0-.055*earw)
    ...
    if C29_GNM_PRESENTATION and not C41_HYPERREAL_NATIVE_EYE and not C42_GEOMETRY_EYE_HAIRLINE:
        for comp,pts in (("left_eye",transformed_landmarks[36:42]),("right_eye",transformed_landmarks[42:48])):
            eye_mesh=c28_gnm_objects.get(comp)
            if eye_mesh is None:
            ...
            iy=front_y+.00020
            _iris_ring_r=.00605 if C40_HYPERREAL_REPAIR else (.00535 if C36_CANONICAL_APPEARANCE else .00505)
            _iris_r=.00555 if C40_HYPERREAL_REPAIR else (.00490 if C36_CANONICAL_APPEARANCE else .00445)
            _pupil_r=.00265 if C40_HYPERREAL_REPAIR else (.00225 if C36_CANONICAL_APPEARANCE else .00185)
            _cornea_r=.00615 if C40_HYPERREAL_REPAIR else (.00545 if C36_CANONICAL_APPEARANCE else .00530)
            cylinder(f"DIGE_C29_{comp.upper()}_IRIS_RING",(ec.x,iy,ec.z),_iris_ring_r,.00015,iris_ring)
            cylinder(f"DIGE_C29_{comp.upper()}_IRIS",(ec.x,iy+.00012,ec.z),_iris_r,.00013,iris)
            cylinder(f"DIGE_C29_{comp.upper()}_PUPIL",(ec.x,iy+.00024,ec.z),_pupil_r,.00011,black)
            cylinder(f"DIGE_C29_{comp.upper()}_CORNEA",(ec.x,iy+.00036,ec.z),_cornea_r,.00010,cornea)
            c29_iris_count+=1
    ...
    target_hairline_z=brow_top+(-.006 if S2_ANATOMY_DYNAMICS else (.008 if S1_ENDOGENOUS else (.018 if C44_FOLLICLE_FLOW else (.026 if C42_GEOMETRY_EYE_HAIRLINE else (.028 if C41_HYPERREAL_NATIVE_EYE else .043)))))
    front=[]
...
c41_groom_metrics={"enabled":False}
if C41_HYPERREAL_NATIVE_EYE:
    if not C28_GNM_HEAD:
        raise RuntimeError("C41 requires GNM head geometry")
    try:
    ...
    surf_rot=groom_surface.matrix_world.to_3x3()
    scalp=[]
    edge_band=[]
    for v in groom_surface.data.vertices:
        ...
        if p.y > eye_center.y-.018 and p.z <= edge_z+.012:
            edge_band.append((v.index,p,n,edge_z))
    if len(scalp) < 650:
        raise RuntimeError(f"C42 scalp coverage too sparse: {len(scalp)}")
    scalp.sort(key=lambda q:q[0])
    edge_band.sort(key=lambda q:(q[1].x,q[0]))

    undercoat=[]
    for vi,root,n,_edge in scalp:
        base_t=Vector((0.0,-1.0,.18))
        base_t=base_t-n*base_t.dot(n)
        if base_t.length<1e-8:
            base_t=Vector((0,-1,.10))
        base_t.normalize()
        cross=n.cross(base_t)
    ...
    for vi,root,n,_edge in roots:
        sx=max(-1.0,min(1.0,root.x/.125))
        sy=max(-1.0,min(1.0,(root.y-eye_center.y)/.11))
        target=bun_center+Vector((.034*sx,.018*sy,-.010+.012*(1.0-abs(sx))))
        scalp_t=Vector((0.0,-1.0,.16))
        scalp_t=scalp_t-n*scalp_t.dot(n)
        if scalp_t.length<1e-8:
            scalp_t=Vector((0,-1,.08))
        scalp_t.normalize()
        p0=root+n*.00055
        p1=p0+scalp_t*.022+n*.0018
        p2=p0+scalp_t*.046+n*.0030+(target-root)*.18
        p3=root.lerp(target,.66)+Vector((0,0,.006))
        primary.append([p0,p1,p2,p3,target])
    curve_object("DIGE_S2_CROWN_GATHER" if S2_ANATOMY_DYNAMICS else ("DIGE_S1_DEEP_SCALP_GATHER" if S1_ENDOGENOUS else ("DIGE_C44_HHIR_DEEP_GATHER" if C44_FOLLICLE_FLOW else "DIGE_C43_HHIR_TANGENTIAL_GATHER")),primary,.000024 if S2_ANATOMY_DYNAMICS else (.000021 if S1_ENDOGENOUS else (.000025 if C44_FOLLICLE_FLOW else .000028)),hair)

    bun=[]
    bun_count=(1050 if S2_ANATOMY_DYNAMICS else (820 if S1_ENDOGENOUS else 520))
    for i in range(bun_count):
        ...
        b1.normalize()
        b2=Vector((-.38*math.sin(phi),.30*math.cos(phi),.94))
        b2=b2-b1*b2.dot(b1)
        if b2.length<1e-8: b2=Vector((0,0,1))
        b2.normalize()
        center=bun_center+Vector((hrng.uniform(-.009,.009),hrng.uniform(-.006,.006),hrng.uniform(-.010,.010)))
        rr=hrng.uniform(.034,.052)
        squash=hrng.uniform(.58,.78)
        pts=[]
        for j in range(19):
            a=math.tau*j/18.0
            pts.append(tuple(center+b1*(rr*math.cos(a))+b2*(rr*squash*math.sin(a))))
        bun.append(pts)
    curve_object("DIGE_S2_LAYERED_BUN" if S2_ANATOMY_DYNAMICS else ("DIGE_S1_LAYERED_ROUNDED_BUN" if S1_ENDOGENOUS else "DIGE_C43_HHIR_ROUNDED_BUN"),bun,.000021 if S2_ANATOMY_DYNAMICS else (.000023 if S1_ENDOGENOUS else .000032),hair)
    ...
    if edge_band:
        ...
        for _vi,root,n,_edge in picks:
            ...
            p0=root+n*.00050
            L=hrng.uniform(.008,.018)
            # Hairline fibers sweep backward/up into the scalp; never down the forehead.
            micro.append([
                p0,
                p0+Vector((side*.0008,-.0030,L*.28)),
    ...
    s1_face_frame_curves=0
    if S1_ENDOGENOUS:
        cap=groom_surface.copy()
        cap.data=groom_surface.data.copy()
        cap.name="DIGE_S1_SCALP_COVERAGE_MASS"
        ...
        if frame_roots:
            ...
            for _vi,root,n,_edge in frame_roots[::step][:frame_target]:
                ...
                for j in range(8):
                    t=j/7.0
                    bend=math.sin(math.pi*t)
                    pts.append(p0+fall*(L*t)+cross*(.0020*math.sin(math.tau*t+phase)*bend)+n*(.0012*bend))
                frame.append(pts)
            if frame:
                curve_object("DIGE_S2_FACE_FRAME_WISPS" if S2_ANATOMY_DYNAMICS else "DIGE_S1_FACE_FRAME_WISPS",frame,.000013 if S2_ANATOMY_DYNAMICS else .000012,hair)
                s1_face_frame_curves=len(frame)
    ...
    c41_groom_metrics={
        ...
        "bun_curves":len(bun),
        "hairline_micro_curves":len(micro),
        "flyaway_curves":len(fly),
        "bun_center":list(bun_center),
        "native_eye_overlay_disabled":True,
    ...
    c39_updo_metrics=c41_groom_metrics
    hair_curve_metrics["c41_groom"]=c41_groom_metrics
...
elif C44_FOLLICLE_FLOW:
    hair_style_label="C44_FOLLICLE_FLOW_GEOMETRY_EYE_V1"
elif C43_TANGENTIAL_GROOM:
    hair_style_label="C43_TANGENTIAL_SCALP_FLOW_GEOMETRY_EYE_V1"
elif C42_GEOMETRY_EYE_HAIRLINE:
    hair_style_label="C42_EDGE_GATED_SCALP_UNDERCOAT_GEOMETRY_EYE_V1"
elif C41_HYPERREAL_NATIVE_EYE:
    hair_style_label="C41_GNM_SCALP_UNDERCOAT_DISTRIBUTED_UPDO_V1"
elif C40_HYPERREAL_REPAIR:
    hair_style_label="C40_HHIR_GNM_SCALP_BOUND_DISTRIBUTED_UPDO_V1"
elif C36_CANONICAL_APPEARANCE and HAIR_LONG_PRIOR:
    hair_style_label="C36_LONG01_AUTHORED_FLOW_STRAND_GROOM_V1"
elif C33_GUIDE_GATED_GROOM:
    hair_style_label="C33_GUIDE_GATED_FIBER_GROOM_V1"
elif C32_GNM_DERMAL_HAIR:
    hair_style_label="C32_GNM_DERMAL_FIBER_GROOM_V1"
elif C29_GNM_PRESENTATION:
    hair_style_label="C29_GNM_STRAND_ONLY_PRESENTATION_V1"
elif C27_SURFACE_FIBERS:
    hair_style_label="C27_OFFICIAL_ANATOMY_SURFACE_FIBERS_V1"
elif C26_PHOTOMETRIC:
    hair_style_label="C26_MPFB2_PHOTOMETRIC_SAFE_GROOM_V1"
elif C25_MATURE_STACK:
    hair_style_label="MPFB2_ENHANCED_SKIN_EYES_CURATED_HAIR_C25_V1"
elif C24_SURFACE_EYES or C24_SOURCE_ALBEDO or C24_HAIRLINE_REPAIR:
    hair_style_label="SURFACE_EYES_SOURCE_ALBEDO_C24_V1"
elif C23_CALIBRATED_EYES or C23_HAIRLINE_REPAIR or C23_FACE_PLANES:
    hair_style_label="CALIBRATED_EYES_FRONTAL_GROOM_C23_V1"
elif C22_PHYSICAL_SKIN or C22_LANDMARK_GROOM or C22_HAIR_MASS_WARP:
    hair_style_label="PHYSICAL_SKIN_LANDMARK_GROOM_C22_V1"
elif C21_NATURAL_DETAIL:
    hair_style_label="HYBRID_BULK_PLUS_NATURAL_MICROHAIRS_C21_V1"
elif C20_VISUAL_REPAIR:
    hair_style_label="HYBRID_BULK_PLUS_MICRO_HAIRLINE_BROW_CURVES_C20_V1"
elif C19_HUMANIZATION:
    hair_style_label="HYBRID_BULK_PLUS_HAIRLINE_CURVES_C19_V1"
elif C18_HYBRID_BULK:
    hair_style_label="HYBRID_BULK_PLUS_BOUNDED_CURVES_C18_V1"
else:
    hair_style_label="HAIR_CURVES_GUIDE_INTERPOLATED_C17_V1"
...
if S1_ENDOGENOUS:
    ...
    tank=_s1_clip_body(
        ...
        s1_tank_mat,.0020 if S2_ANATOMY_DYNAMICS else .0017
    )
...
def area(name,loc,energy,size,color,target=(0,0,1.25)):
    ...
    return o
if S2_ANATOMY_DYNAMICS:
    face_target=(0,.010,1.54)
    area("S2_KEY",(1.45,1.95,2.20),235,1.85,(1.0,.96,.92),face_target)
    area("S2_FILL",(-1.65,2.05,1.68),48,2.75,(.92,.95,1.0),face_target)
    area("S2_RIM",(-.50,-1.80,2.22),78,1.45,(1.0,.86,.75),face_target)
    area("S2_TOP",(0,.10,2.85),44,1.40,(1.0,.97,.94),face_target)
elif S1_ENDOGENOUS:
    face_target=(0,.010,1.55)
    area("S1_KEY",(1.55,2.15,2.35),185,2.25,(1.0,.95,.91),face_target)
    area("S1_FILL",(-1.80,2.10,1.70),42,3.00,(.91,.95,1.0),face_target)
    area("S1_RIM",(-.55,-1.95,2.30),64,1.80,(1.0,.84,.72),face_target)
    area("S1_TOP",(0,.15,3.00),72,1.65,(1.0,.96,.90),face_target)
elif C42_GEOMETRY_EYE_HAIRLINE:
    face_target=(0,.012,1.585)
    area("KEY",(1.20,1.72,2.06),138,1.45,(1.0,.95,.91),face_target)
    area("FILL",(-1.50,1.95,1.76),7,2.55,(.90,.94,1.0),face_target)
    area("RIM",(-.18,-1.70,2.08),25,1.30,(1.0,.84,.74),face_target)
    area("DETAIL",(-.65,.88,1.68),4,.50,(1.0,.99,.97),face_target)
elif C41_HYPERREAL_NATIVE_EYE:
    face_target=(0,.012,1.585)
    area("KEY",(1.25,1.75,2.08),132,1.50,(1.0,.95,.91),face_target)
    area("FILL",(-1.55,1.95,1.76),8,2.45,(.90,.94,1.0),face_target)
    area("RIM",(-.18,-1.72,2.10),28,1.35,(1.0,.84,.74),face_target)
    area("DETAIL",(-.68,.90,1.68),5,.52,(1.0,.99,.97),face_target)
elif C40_HYPERREAL_REPAIR:
    face_target=(0,.012,1.585)
    area("KEY",(1.35,1.85,2.10),118,1.75,(1.0,.92,.87),face_target)
    area("FILL",(-1.55,1.90,1.80),18,2.70,(.88,.93,1.0),face_target)
    area("RIM",(-.20,-1.75,2.12),32,1.45,(1.0,.80,.68),face_target)
    area("DETAIL",(-.72,.98,1.70),7,.62,(1.0,.98,.96),face_target)
...
bg.inputs["Color"].default_value=((0.016,0.018,0.022,1) if S2_ANATOMY_DYNAMICS else ((0.018,0.021,0.028,1) if S1_ENDOGENOUS else ((0.030,0.020,0.018,1) if C36_CANONICAL_APPEARANCE else (0.012,0.014,0.020,1))))
bg.inputs["Strength"].default_value=(.030 if S2_ANATOMY_DYNAMICS else (.045 if S1_ENDOGENOUS else (.11 if C36_CANONICAL_APPEARANCE else (.022 if C26_PHOTOMETRIC else .055))))
...
# Distances inherit the latest provider-backed Drive sweep: V6C-122 fullbody=5.1m.
# C39.1 must define its camera before the HERO_ONLY search render; the prior C39
# override happened after the preview was already emitted and therefore recorded 70mm.
if S2_ANATOMY_DYNAMICS:
    hero_view=("04_HERO85",(0,.82,1.595),(0,.010,1.575),85,2.8,1024,1024)
elif S1_ENDOGENOUS:
    hero_view=("04_HERO85",(0,1.02,1.600),(0,.010,1.580),85,4.0,1024,1024)
elif C42_GEOMETRY_EYE_HAIRLINE:
    hero_view=("04_HERO85",(0,1.00,1.598),(0,.012,1.585),85,4.0,900,900)
elif C41_HYPERREAL_NATIVE_EYE:
    hero_view=("04_HERO85",(0,.98,1.598),(0,.012,1.585),85,4.0,900,900)
elif C39_HHIR_HYPERREAL:
    hero_view=("04_HERO85",(0,.96,1.598),(0,.012,1.585),85,3.6,900,900)
elif C36_CANONICAL_APPEARANCE:
    hero_view=("04_HERO85",(0,.82,1.598),(0,.010,1.585),70,3.2,900,900)
else:
    hero_view=("04_HERO85",(0,1.10,1.595),(0,.030,1.580),85,4.5,900,900)
all_views=[
 ("01_FRONT50",(0,(3.75 if S2_ANATOMY_DYNAMICS else 5.1),1.02),(0,0,.92),50,(5.6 if S2_ANATOMY_DYNAMICS else 7.1),(720 if S2_ANATOMY_DYNAMICS else (640 if S1_ENDOGENOUS else 512)),(1080 if S2_ANATOMY_DYNAMICS else (960 if S1_ENDOGENOUS else 768))),
 ("02_LEFT_PROFILE50",((3.75 if S2_ANATOMY_DYNAMICS else 5.1),0,1.07),(0,0,.97),50,(5.6 if S2_ANATOMY_DYNAMICS else 7.1),(720 if S2_ANATOMY_DYNAMICS else (640 if S1_ENDOGENOUS else 512)),(1080 if S2_ANATOMY_DYNAMICS else (960 if S1_ENDOGENOUS else 768))),
 ("03_THREE_QUARTER50",((2.65 if S2_ANATOMY_DYNAMICS else 3.60),(2.65 if S2_ANATOMY_DYNAMICS else 3.60),1.08),(0,0,.98),50,(5.0 if S2_ANATOMY_DYNAMICS else 6.3),(720 if S2_ANATOMY_DYNAMICS else (640 if S1_ENDOGENOUS else 512)),(1080 if S2_ANATOMY_DYNAMICS else (960 if S1_ENDOGENOUS else 768))),
 hero_view,
 ("05_BACK_THREE_QUARTER50",(-(2.65 if S2_ANATOMY_DYNAMICS else 3.60),-(2.65 if S2_ANATOMY_DYNAMICS else 3.60),1.07),(0,0,.98),50,(5.0 if S2_ANATOMY_DYNAMICS else 6.3),(720 if S2_ANATOMY_DYNAMICS else (640 if S1_ENDOGENOUS else 512)),(1080 if S2_ANATOMY_DYNAMICS else (960 if S1_ENDOGENOUS else 768))),
 ("06_BACK50",(0,-(3.75 if S2_ANATOMY_DYNAMICS else 5.1),1.02),(0,0,.92),50,(5.6 if S2_ANATOMY_DYNAMICS else 7.1),(720 if S2_ANATOMY_DYNAMICS else (640 if S1_ENDOGENOUS else 512)),(1080 if S2_ANATOMY_DYNAMICS else (960 if S1_ENDOGENOUS else 768)))
]
...
if S2_ANATOMY_DYNAMICS:
    aim((0,.82,1.595),(0,.010,1.575),85,2.8)
elif S1_ENDOGENOUS:
    aim((0,1.02,1.600),(0,.010,1.580),85,4.0)
elif C42_GEOMETRY_EYE_HAIRLINE:
    aim((0,1.00,1.598),(0,.012,1.585),85,4.0)
elif C41_HYPERREAL_NATIVE_EYE:
    aim((0,.98,1.598),(0,.012,1.585),85,4.0)
elif C39_HHIR_HYPERREAL:
...
receipt={
 ...
 "appearance_candidate":(
   ...
   "C43_HHIR_TANGENTIAL_GROOM_V1" if C43_TANGENTIAL_GROOM else
   "C42_HHIR_GEOMETRY_EYE_EDGE_HAIRLINE_V1" if C42_GEOMETRY_EYE_HAIRLINE else
   "C41_HHIR_NATIVE_EYE_SCALP_COVER_V1" if C41_HYPERREAL_NATIVE_EYE else
   "C40_HHIR_HYPERREAL_SCALP_BOUND_UPDO_V1" if C40_HYPERREAL_REPAIR else
   "C39_1_HHIR_HYPERREAL_UPDO_CAMERA_FIXED_V1" if C39_HHIR_HYPERREAL else
 ...
 "appearance_selection":{
   ...
   "c40_hyperreal_repair":C40_HYPERREAL_REPAIR,
   "c41_hyperreal_native_eye":C41_HYPERREAL_NATIVE_EYE,
   "c42_geometry_eye_hairline":C42_GEOMETRY_EYE_HAIRLINE,
   ...
   "s2_control_contract":{
      ...
      "tmj_jaw":"NEUTRAL_LIP_SEAL_EXECUTED_OPEN_CLOSE_NOT_EXECUTED",
      "soft_tissue":"STATIC_REST_FIELD_EXECUTED_DYNAMIC_SEQUENCE_NOT_EXECUTED",
      "wind":"ZERO_WIND_SECONDARY_FLOW_EXECUTED_5MPS_SEQUENCE_NOT_EXECUTED",
      "camera":{"sensor_mm":36,"body_lens_mm":50,"body_fstop":5.6,"hero_lens_mm":85,"hero_fstop":2.8}
   },
 ...
 "claim_ceiling":("S2_ANATOMY_AWARE_SOURCE_LEVEL_PREVIEW_EXECUTED; STATIC_REST_FIELDS_ONLY; VISUAL_AUDIT_REQUIRED; HAND_JAW_SOFT_TISSUE_WIND_DYNAMICS_NOT_EXECUTED" if S2_ANATOMY_DYNAMICS else ("S1_ENDOGENOUS_SOURCE_LEVEL_SYSTEM_EXECUTED; VISUAL_HYPERREAL_CERTIFICATION_REQUIRES_DIRECT_MULTIVIEW_AUDIT" if S1_ENDOGENOUS else "PRODUCTION_TOPOLOGY_PLUS_CYCLES_PATH_TRACED_CANDIDATE; HYPERREAL_CERTIFICATION_REQUIRES_VISUAL_AND_LHYPER_AUDIT; DEUS_IDENTITY_NOT_BOUND"))
}
